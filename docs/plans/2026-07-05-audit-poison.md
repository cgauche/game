...
- **Fix** : Remplacer « Pacte du Marteau, LDB 17 l.68 » par « Chance, LDB 17 l.24/84 » (aligné sur l'attribution correcte déjà utilisée dans rollFlowFactory.ts).
...
- **Affirme** : Contre une cible Inconsciente, le RAW (« Je ne faillirai pas ! », LDB 17 l.68) permettrait normalement à l'attaquant de CHOISIR la Localisation touchée (comme pour tout résultat forcé) ; le code documente que ce choix n'est pas modélisé — la localisation reste tirée au hasard depuis le jet brut.

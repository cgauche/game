...
| `.chip.tone-warn` / `.chip.tone-danger` | Variantes de TON du chip (or / rouge alerte) | Bande d'alarmes de la colonne moniteur (#492) — un chip qui SIGNALE (pas un badge neutre) prend le ton de sa gravité ; composer, jamais un `style={{color}}` inline. |

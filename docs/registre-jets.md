...
| `src/state/combatFlow.ts` | 2 | mixte | 3343, 7890 | 1 gate de main (`pendingHandGate`, `openAttackCascade`) monté à la main -> #1064 ; 1 `PendingReload` d'ennemi construit APRÈS un `rollSansPilote` déjà scellé — canonique : objet de RENDU (journal/popin), aucun jet à ouvrir. |
...
| `src/state/store.ts` | 1 | canonique | 2838 | canonique : re-ciblage d'un `pendingTest` EXISTANT (`{ ...pt, … }`) sur un autre candidat — `target` recopié du candidat DÉJÀ calculé par la fabrique, aucun jet neuf décrit. |
...
<!-- sources-empreinte: f938812e6a7943bd82ce64b83d997642078c6948 (2094 fichiers, 134 dossiers) corps: 26c9095519a9824aac66e9a53ac3bdd42b924d6e -->

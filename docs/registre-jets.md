...
| `src/state/combatFlow.ts` | 2 | mixte | 2842, 6834 | 1 gate de main (`pendingHandGate`, `openAttackCascade`) monté à la main -> #1064 ; 1 `PendingReload` d'ennemi construit APRÈS un `rollSansPilote` déjà scellé — canonique : objet de RENDU (journal/popin), aucun jet à ouvrir. |
| `src/state/combatSlice.ts` | 5 | dette | 1844, 1890, 1987, 2404, 2802 | 2 `pendingReload` (pièce servie / poste de navire), 1 `pendingStateRecovery`, 1 `pendingHandGate` (2ᵉ main), 1 `pendingHeal` -> #1064 (le lot d'affichage les re-route ; 6 -> 5 : le `pendingTest` de `battleGainAdvantage` passe par `openSkillTest`). |
| `src/state/interludeFlow.ts` | 1 | dette | 733 | `pendingActivity` du catalogue d'Activités (`openCatalogActivity`) — fabrique UNIQUE de toutes les Activités à jet d'interlude -> #1064. |
...
| `src/state/seaVoyageFlow.ts` | 1 | dette | 2002 | `pendingSteamSave` (`openSteamSave`, Test d'Initiative de l'ingénieur) -> #1064. |
| `src/state/store.ts` | 1 | canonique | 2427 | canonique : re-ciblage d'un `pendingTest` EXISTANT (`{ ...pt, … }`) sur un autre candidat — `target` recopié du candidat DÉJÀ calculé par la fabrique, aucun jet neuf décrit. |
...
_85 call-sites mesurés dans 19 fichiers, pour 82 exports rouleurs dérivés de `src/engine` — par nature : 62 mixte, 13 dette, 10 canonique._

...
| `src/state/combatFlow.ts` | 2 | mixte | 2980, 7213 | 1 gate de main (`pendingHandGate`, `openAttackCascade`) monté à la main -> #1064 ; 1 `PendingReload` d'ennemi construit APRÈS un `rollSansPilote` déjà scellé — canonique : objet de RENDU (journal/popin), aucun jet à ouvrir. |
...
| `src/state/merchantFlow.ts` | 1 | dette | 863 | `pendingAppraise` (Évaluation / Intuition de détection) -> #1064. |
| `src/state/seaVoyageFlow.ts` | 1 | dette | 2030 | `pendingSteamSave` (`openSteamSave`, Test d'Initiative de l'ingénieur) -> #1064. |
| `src/state/store.ts` | 1 | canonique | 2444 | canonique : re-ciblage d'un `pendingTest` EXISTANT (`{ ...pt, … }`) sur un autre candidat — `target` recopié du candidat DÉJÀ calculé par la fabrique, aucun jet neuf décrit. |

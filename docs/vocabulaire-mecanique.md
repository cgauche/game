...
| Durée, horloge, effet différé, expiration | `charMod`, `gainResource`, `castPenalty`, `statusMod`, `grantReverseToken`, `grantTrait`, `augmentWeapon`, `reduceDiseaseDays`, `contractDisease`, `suppressPsych`, `grantNaturalWeapon`, `perRound`, `scheduleRespawn`, `polymorph`, `transform`, `suppressSymptom`, `delayed` |
...
| `grantTalent` | `talentId`, `spec?` | exécutée | `engine/corruption.ts`, `engine/talentEffects.ts`, `state/aiSpellValue.ts` +1 | 57 — `mutations.json:tete-bestiale-chien`, `mutations.json:fuite-aethyrique` … | Talent OCTROYÉ par un effet : porté par l'`ActiveEffect`, lu par le registre `combatFeatures` (`featuresOf` réunit `c.talents` ET `activeEffects[].grantedTalent`) — donc PLEINEMENT effectif sans être posé dans `c.talents` (fiche/avancement intacts). |
...
| `rollTable` | `die`, `addNegativeSL?`, `extraRollsPerStep?`, `rows` \| `die?`, `addNegativeSL?`, `extraRollsPerStep?`, `tableId` | exécutée | `engine/miscast.ts`, `engine/overcast.ts` | 11 — `spells.json:allure-demoniaque`, `spells.json:allure-demoniaque` … | Tirage sur TABLE (`die` = d10/d100) : lookup par fourchette `[min,max]` (`findTableEntry`, source unique), les `ops` de la rangée touchée sont appliquées avec le MÊME ctx. |
...
| `wounds` | `amount`, `perSL?`, `onlyGroups?`, `ignoreTB?`, `ignoreAP?`, `bypassArmour?`, `apFrom?`, `min?`, `extraAP?`, `weaponHit?` | exécutée | `engine/aaCritical.ts`, `engine/critical.ts`, `engine/disease.ts` +11 | 149 — `criticals.json:blessure-spectaculaire`, `criticals.json:coupure-mineure` … | Blessures subies DIRECTEMENT. |

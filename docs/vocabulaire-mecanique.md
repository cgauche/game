...
| `gainAdvantage` | `amount`, `feedOpposingPool?` | exécutée | `state/aiSpellValue.ts`, `state/targetingModes.ts` | 2 — `tavernGames.json:bras-de-fer`, `traits.json:redoutable` | Porte l'Avantage de la cible à AU MOINS `amount` (jamais réduit). |

...
| Magie, incantation, prière, miracle, contrecoup | `ap`, `gainResource`, `castPenalty`, `grantTrait`, `grantTalent`, `augmentWeapon`, `freeReroll`, `critTwice`, `suppressPsych`, `castWard`, `domeWard`, `attackWardFM`, `noHunger`, `weatherWard`, `grantWeapon`, `interruptFocus`, `perRound`, `charDamage`, `zone`, `transform`, `attackKeyword`, `mitigateIncoming` |
...
| Psychologie : Peur, Terreur, Frénésie, Animosité, Obsession | `endPsych`, `grantTrait`, `grantPsychTrait`, `removePsychTrait`, `grantTalent`, `grantCareerTalent`, `suppressPsych`, `ignoreAnimosity`, `grantFreeAttack`, `sbBonus` |
...
| `armourPierce` | `amount` | **inerte au switch** | `engine/qualities/dispatch.ts`, `state/targetingModes.ts` | 1 — `qualities.json:perforante` | PASSIF d'ARME : Perforante (LDB 62) — ignore `amount` PA (+ la matière non-métal) à la mitigation. |
...
| `corruption` | `amount`, `perSL?`, `align?` | exécutée | `engine/miscast.ts`, `state/aiSpellValue.ts`, `state/targetingModes.ts` | 11 — `miscast.json:mineure-murmures-mortels`, `miscast.json:mineure-malediction-de-corruption` … | Points de Corruption (LDB 19). |
...
| `exposeDisease` | `disease`, `difficultyShift?`, `incubation?` | exécutée | `state/aiSpellValue.ts` | 4 — `qualities.json:infecte`, `traits.json:infecte` … | EXPOSE la cible à une Maladie (`disease` = id de `maladies.json`) → Test de Contraction au bilan de fin de combat (LDB 20 l.32/49). |
...
| `gainAdvantage` | `amount`, `feedOpposingPool?` | exécutée | `state/aiSpellValue.ts`, `state/targetingModes.ts` | 1 — `traits.json:redoutable` | Porte l'Avantage de la cible à AU MOINS `amount` (jamais réduit). |
...
| `giveTrapping` | `trappingId?`, `custom?`, `count?`, `perSL?` | exécutée | `state/aiSpellValue.ts`, `state/targetingModes.ts` | 2 — `spells.json:generosite-de-manann`, `spells.json:recolte-de-rhya` | Crée un objet (`trapping`) dans l'inventaire de la cible — nom RÉEL de la base → objet à stats, nom inconnu → objet CUSTOM (misc). |
...
| `grantTalent` | `talentId`, `spec?` | exécutée | `engine/corruption.ts`, `engine/talentEffects.ts`, `state/aiSpellValue.ts` +1 | 57 — `mutations.json:tete-bestiale-chien`, `mutations.json:fuite-aethyrique` … | Talent TEMPORISÉ (Jalon 2.6 — « +1 Talent Sans peur tant que le Sort est actif ») : porté par l'ActiveEffect, lu par le registre `combatFeatures` (featuresOf) — PAS posé dans `c.talents` (fiche/avancement intacts). |
| `grantTrait` | `traitId`, `arg?`, `argFrom?`, `indice?`, `indicePerSL?`, `onlyGroups?`, `durationRounds?` | exécutée | `engine/corruption.ts`, `engine/polymorph.ts`, `state/aiSpellValue.ts` +1 | 130 — `domains.json:bete`, `maneuvers.json:forme-hybride-ulric` … | Trait de créature TEMPORISÉ (Jalon 2.6 — « vous gagnez le Trait X tant que le Sort est actif ») : posé dans `c.traits` (vu par TOUS les consommateurs — dispatch, psy, IA, déplacement), retiré à l'expiration de l'ActiveEffect porteur. |
...
| `heal` | `amount`, `perSL?` | exécutée | `state/aiSpellValue.ts`, `state/massBattleFlow.ts`, `state/rollSeam.ts` +2 | 15 — `spells.json:benediction-de-guerison`, `spells.json:cauteriser` … | Blessures rendues (plafonnées au max). |
...
| `light` | `radiusTiles`, `durationRounds?` | exécutée | — | 7 — `spells.json:lumiere`, `spells.json:edifice-illumine` … | Émission de LUMIÈRE (rayon en cases, brouillard de guerre, 1 case=2 m). |
...
| `maxWeaponHands` | `hands`, `durationRounds?` | exécutée | `engine/critical.ts`, `engine/trauma.ts` | 9 — `aa-criticals.json:aa-bras-11`, `aa-criticals.json:aa-bras-51` … | Plafond de mains d'arme maniables — GÉNÉRALISE `noTwoHanded` (hands:1 = pas d'arme à deux mains). |
...
| `narrative` | `text` | exécutée | `engine/polymorph.ts`, `engine/spellspec.ts` | 486 — `spells.json:benediction-de-conscience`, `spells.json:alerte` … | Effet non modélisé : journalisé verbatim, arbitrage MJ (rien d'inventé). |
...
| `removeCondition` | `id?`, `value?`, `valuePerSL?`, `all?` | exécutée | `state/aiSpellValue.ts`, `state/instanceIdMigration.ts`, `state/targetingModes.ts` | 29 — `domains.json:extenue`, `domains.json:hemorragique` … | Retrait d'États : `id` absent = au choix de la cible (1er État porté). |
...
| `rollTable` | `die`, `addNegativeSL?`, `extraRollsPerStep?`, `rows` \| `die?`, `addNegativeSL?`, `extraRollsPerStep?`, `tableId` | exécutée | `engine/overcast.ts` | 11 — `spells.json:allure-demoniaque`, `spells.json:allure-demoniaque` … | Tirage sur TABLE (`die` = d10/d100) : lookup par fourchette `[min,max]` (`findTableEntry`, source unique), les `ops` de la rangée touchée sont appliquées avec le MÊME ctx. |
...
| `sbBonus` | `amount` | **hors switch** | `engine/trauma.ts` | 1 — `psychology.json:frenesie` | +N au Bonus de Force employé aux DÉGÂTS (Frénésie : +1 « grâce à votre férocité », LDB 21 l.34). |
...
| `skillDRBonus` | `skill?`, `bonus`, `spec?`, `testType?` | exécutée | `engine/navalTraits.ts`, `state/targetingModes.ts` | 55 — `naval-traits.json:peu-maniable`, `naval-traits.json:peu-maniable` … | +N DR à un Test de Compétence nommé (Furtif : +Bonus d'Agilité au DR de Discrétion, LDB 85 p.339 ; chanson « Jacques Bret » : +1 DR sur tout Test de Corps à corps réussi, MDG 09 l.228). |
| `skillMod` | `skill`, `mod`, `sense?` | exécutée | `engine/trauma.ts`, `state/aiSpellValue.ts` | 29 — `drunkenness.json:bravoure-marienburgher`, `mutations.json:langue-pendante` … | Modificateur (pénalité/bonus) à UNE Compétence nommée — GÉNÉRALISE les pénalités de séquelle `skillPenalty` (Langue −100 « auto-échec parole ») ET `dodgePenalty` (Esquive −20, mobilité). |
...
| `summon` | `ref`, `count`, `countPerSL?`, `addTraits?`, `size?`, `allyOfCaster?`, `despawnIfCasterDown?` | **inerte au switch** | `state/aiSpellValue.ts`, `state/combatFlow.ts`, `state/summonFlow.ts` | 11 — `spells.json:destrier-d-ombre`, `spells.json:menace-rampante` … | INVOCATION de créature(s) (Nécromancie « Réanimation/Relever les morts », Ulric « Hurlement du loup », Démonologie « Manifestation », Taal « Roi de la Nature »…). |
...
| `testMod` | `amount`, `char?`, `combatOnly?`, `movementOnly?`, `hearingOnly?`, `exceptSkills?`, `weaponHand?` | exécutée | `engine/conditions.ts`, `engine/qualities/dispatch.ts`, `engine/trauma.ts` +1 | 26 — `aa-criticals.json:aa-bras-96`, `aa-criticals.json:aa-jambe-96` … | Modificateur de Test du porteur (Malédiction de malchance : −10 global). |
...
| `weaponDamageMod` | `dr?`, `mode?`, `plusUnits?`, `negateAtouts?`, `chargeGated?` | **inerte au switch** | `engine/qualities/dispatch.ts` | 6 — `qualities.json:devastatrice`, `qualities.json:percutante` … | PASSIF d'ARME : modificateur de DÉGÂTS (LDB 62-63) — Pointue (+1 DR `dr`), Dévastatrice (DR = max(DR, dé des unités), `mode:'maxUnits'`), Percutante (+ dé des unités, `plusUnits`), Inoffensive (annule les Atouts de Dégâts, `negateAtouts`), Épuisante (`chargeGated` : Percutante/Dévastatrice de l'arme inertes hors Charge). |
| `weaponRollMod` | `phase`, `drMod?`, `flatMod?` | **inerte au switch** | `engine/qualities/dispatch.ts` | 9 — `qualities.json:a-enroulement`, `qualities.json:defensive` … | PASSIF d'ARME (Atout/Défaut, LDB 62-63) : modificateur de DR/plat à une PHASE de jet de combat — Précise (+10 `flatMod` en attaque), Imprécise (−1 DR en attaque), Défensive (+1 DR parade du défenseur), À Enroulement (−1 DR parade adverse), Lente (+1 DR à TOUTE défense adverse), Pratique/Peu Fiable (±1 DR à un Test raté). |
...
| `wounds` | `amount`, `perSL?`, `onlyGroups?`, `ignoreTB?`, `ignoreAP?`, `bypassArmour?`, `apFrom?`, `min?`, `extraAP?`, `weaponHit?` | exécutée | `engine/aaCritical.ts`, `engine/critical.ts`, `engine/disease.ts` +11 | 147 — `criticals.json:blessure-spectaculaire`, `criticals.json:coupure-mineure` … | Blessures subies DIRECTEMENT. |
| `zone` | `shape`, `radiusMeters?`, `lengthMeters?`, `lengthPerSL?`, `blocksLoS?`, `onCross?`, `perRound?`, `crossTest?`, `barrier?`, `gate?`, `noCorruption?` | **inerte au switch** | `engine/overcast.ts`, `state/combatFlow.ts`, `state/zones.ts` | 12 — `spells.json:vol-du-destin`, `spells.json:grands-feux-d-u-zhul` … | ZONE PERSISTANTE posée par le sort (Mur de feu, Grands feux d'U'Zhul, Vol du Destin). |
...
| `foeInLoS` | — | Y a-t-il un adversaire VIVANT dans la Ligne de Vue de `target` (`ctx.foeInLoS`) ? Géométrie d'arène emballée en donnée (au-dessus de `lineOfSightCover`) : sortie de Frénésie « plus d'ennemi en vue → fin » (LDB 21 l.36), fuite/récupération du Brisé « hors de vue de l'ennemi » (LDB 16 l.55). |
| `hiddenFromFoes` | — | Aucun adversaire VIVANT ne voit l'acteur (sens foe→acteur, ≠ `foeInLoS` acteur→foe) — « caché hors de vue de l'ennemi » (Brisé, LDB 16 l.55/71 : retrait sans Test + difficulté Accessible). |
| `engaged` | — | L'acteur est-il ENGAGÉ avec un adversaire (LDB 13 l.159) ? Gate de récupération du Brisé (LDB 16 l.57 : aucun Test si Engagé). |

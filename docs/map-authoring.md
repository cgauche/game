...
| `seatAssignments?: SeatAssignments` | qui est attablé où : `propId → slotId → occupant` (`{kind:'entity',entityId}` ou `{kind:'party',rang}`). Ids **FIXES** seulement — `propId` comme `entityId` doivent figurer littéralement dans `entities` ; un occupant de groupe se désigne par **RANG** (voir ci-dessous) |
...
- Un occupant du GROUPE est un **EMPLACEMENT**, pas un personnage : `{ kind: 'party', rang }` avec `1 ≤ rang ≤ PARTY_MAX` (`src/state/combatants.ts`). Un document ne peut pas nommer un héros que le joueur créera plus tard ; « Héros 2 » désigne le deuxième emplacement du groupe, et le runtime y résout `party[rang - 1]`. Un rang hors borne est une ERREUR de document ; un rang que le groupe courant n'atteint pas n'en est pas une — il s'élague simplement au chargement.
...
- La `pos` d'un PNJ attablé **est** la case d'abord résolue de sa place (position logique : c'est de là qu'il s'assoit et là qu'il se relève ; l'ancre fractionnaire n'est que du rendu), et cet abord doit être PRATICABLE — un meuble cerné de murs n'offre aucune place. Le prédicat est unique (`seatIsOccupiable`, `src/state/seating.ts`) : ce que le geste refuse, `validateScene` et `buildScene` le refusent aussi. `buildScene` échoue fail-fast dessus.
- À la souris, ce champ s'authore dans l'inspecteur de l'éditeur : sélectionne le décor → rubrique « Places assises » (`src/ui/editor/SeatAssignmentsField.tsx`). Toute mutation d'entité de l'éditeur (position, orientation, étage, `ref`, suppression — meuble OU corps) traverse le SEAM UNIQUE `normaliseAssises` (`src/state/sceneEdit.ts`) : les corps attablés se recalent sur les abords re-résolus, et une place devenue inoccupable est levée dans la même mutation. Garde structurelle : `src/ui/editor/seam-assise-guard.test.ts`.

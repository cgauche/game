...
(`src/engine/trauma.ts:884`), le mode de combinaison (le seul kind ADDITIF reconnu par
...
Table des annulateurs : `PASSIVE_CANCELLERS` (`src/engine/trauma.ts:884`) — elle doit rester
...
`passiveMods` (`src/engine/trauma.ts:983`) est le **point de
...
|---|---|---|---|
| `src/engine/trauma.ts:985` | — | `traumaPassiveMods` | — |
| `src/engine/trauma.ts:991` | — | `diseasePassiveOps` | Maladies (kind `maladie`, annulée par Détermination ; passifs des symptômes via `diseasePassiveOps`) + Faim (kind `faim`, non annulée : `noHunger` purge l'état à l'entretien, pas ici). |
...
| `src/engine/trauma.ts:1029` | — | `qualityWearMods` | — |
...
| `talentPassiveMods` | `src/engine/talentEffects.ts:249` | Modificateurs PASSIFS continus des talents POSSÉDÉS (`TalentData.passive` : Coup puissant, Dur à cuire…, ou Frénésie → `grantFreeAttack`) en `GameOp[]`, émis kind `intrinseque` et RÉPÉTÉS par niveau (`t.times`). |
| `traitPassiveMods` | `src/engine/traits/dispatch.ts:177` | PassiveMod[] de PROFIL des traits — la DONNÉE éditable `TraitData.passive` (vocab GameOp unifié, éditée par GameOpEditor comme un sort). |
...
<!-- sources-empreinte: 2b6d7242278b26921867d88ea0eb21500f02bbab (144 fichiers, 1 dossiers) corps: fcd5d679b45e3bbbce636cc532507010a6af2502 -->

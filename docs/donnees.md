...
(`24` fichier(s) exempt(s) sur `122`). Aucune de ces valeurs n'est écrite ici :
...
| `buildings.json` | LES types de bâtiment — empreinte offerte à la pose, couverture par défaut des nappes de toit, ornements d'identité posés en billboard ; lu par la façade `src/gameIso/catalog/buildings` (7 entrée(s)) | `buildings` — dataset `buildings` |
...
- **`variants?: Variant[]`** (#563/#564) — variante RÉGLÉE d'une entrée sous une **règle optionnelle**
  ...
  - `desc`/`source` — Codex, `effectiveEntry`, `src/ui/compendium/registry.ts:1469`
  ...
  - `desc`/`source` — Codex, `effectiveEntry`, `src/ui/compendium/registry.ts:593`
...
<!-- sources-empreinte: d289f6f45b08d4db0bd61f17a3737a7cf0e9f727 (361 fichiers, 2 dossiers) corps: b632d4cb72e146704691efad6725b0cabe230183 -->

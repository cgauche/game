...
réfute au besoin : (1) bijection stricte manifeste ⇄ `112` fichiers réels de
...
schéma zod (`112/112`, cf. §E-bis) ; (4) présence effective sur disque de
...
| `regles.json` | Procédures / options de jeu au texte VERBATIM (Sombre Pacte, modes d'attaque/défense, Empoignade, Focalisation étendue, Ragot au marché…) — routées en tooltip `CodexRef` (catégorie Codex `regles`), jamais une paraphrase de règle (#392) (66 entrée(s)) |
...
**112/112** datasets sous contrat, décompte CALCULÉ des defs présentes dans

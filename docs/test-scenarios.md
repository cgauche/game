...
| Combat | Poursuite terrestre | L'Effet `startPursuit` (#95, LDB 15 l.87-109) ouvre la boucle de manches JOUABLE (cascade influençable purpose:'pursuite', un Test de Mouvement par héros) dès l'entrée ; rattrapage (Distance ≤ 0) bascule en combat contre les mêmes brigands, évasion (Distance ≥ 10) se dénoue au récit. | Groupe fixe (pré-tirés) |
...
| Rendu | Galerie de modèles | Tous les modèles de l’IsoStage : {creatures.length} créatures (empreintes par Taille) + TOUTES les carrières ({careers.length}) + TOUTES les armes ({weapons.length}) + mutants + démo Monstrueuse. Exploration, SANS combat. | Exploration libre, aucun combat |

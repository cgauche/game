...
| ⚔️ Combat | 🧭 Poursuite terrestre | l'Effet `startPursuit` (#95, LDB 15 l.87-109) ouvre la boucle de manches JOUABLE dès l'entrée (cascade influençable `purpose:'pursuite'`) : le groupe fuit 3 brigands, rattrapage (Distance ≤ 0) bascule en combat, évasion se dénoue au récit |
...
| 🐲 Créatures | 🐲 Aspersion — créature marine hors de l'eau | Action « Asperger d'eau » (MDG 16 l.19, #497) : une anguille mâcheprise ALLIÉE (`side:'ally'` → `kind:'hero'`, seul camp accepté par `waterSprayCandidates`) échouée sur la terre (`offTerrain`, aucune tuile d'eau, adjacente AU SPAWN) suffoque Round après Round (`suffocationTick`, #477) sauf si le Soldat, outre à eau en poche, l'asperge (`battleWater`, aucun jet) ; victoire `surviveRounds` (aucun ennemi requis) |
...
| 🛒 Marché | 🛒 Marché & équipement | Acheter/Vendre/Marchander/Évaluer/Réparer, 3 archétypes (armurier + herboriste via dialogue + maquignon), écran d'EMPLACEMENTS (couches d'armure, sets d'armes) |
| 🛒 Marché | 🛳️ Commerce fluvial (le Reik) | commerce de cargaison MSRC ch.11 JOUABLE : le Reik peuplé de vraies localités marchandes (Index géographique, Taille/Richesse/Produits verbatim) reliées par routes de BARGE ; boucle du marchand acheter/descendre le fleuve/revendre (profit), Marchandage/Évaluation, exposition hydrique (MSRC ch.14) en approchant d'Altdorf |

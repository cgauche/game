...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `MDG 16` (l.15-19, l.63-305) → `scene`, `placeCombatant`, `scenario`, `fireTurnEdgeTriggers`, `effectiveMovement`, `suffocationTick`, `reconcileAdvantageToPool`, `OP_DEFS`, `creditOpposingAdvantage`, `sea-scorbut`, +34 — `src/data/creatures.json`, `src/data/domains.json`, `src/data/index.ts`, `src/data/schemas/grammaire/mecanique.ts`, `src/data/skills.json`, `src/data/voyage-stakes.json`, +14 fichiers

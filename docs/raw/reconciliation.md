...
- `src/data/skills.json:1812` (l.736) — "note": "EDO 10 l.736"

### MCLB 2 — 4 réf(s) code, 0 dans l'Atlas
- `src/data/skills.json:937` (l.1844) — "note": "MCLB 02 l.1844"
- `src/data/skills.json:1518` (l.1322) — "note": "MCLB 02 l.1322"
- `src/data/skills.json:1528` (l.2426) — "note": "MCLB 02 l.2426"
...
- `src/state/riverVoyageFlow.ts:1095` (l.71) — // Sur la rivière on peut mouiller le long de la berge : coucher À BORD (hamacs, MDG 03 l.71) offert
...
- `src/state/scene.ts:100` (l.72) — *  Le patron est AUTHORÉ dans la source, pas inventé : `NADJ 04 l.72` — « Elle jouera une partie de
...
- `src/data/index.ts:785` (l.5) — *  Portée sur `gnomes` (`NADJ 14 l.5`, règle `creation-gnome-jouable`). */
- `src/data/reglesOptionnelles.json:441` (l.5) — "ref": "NADJ 14 l.5",
...
- l.85 — `src/data/index.ts:2379` — *  = `totale` (MDG 12 l.364 / MSRC 12 l.85), Plat-bord = `moyenne` (MSRC 12 l.111). Géométrie de Pont,
...
- l.111 — `src/data/index.ts:2379` — *  = `totale` (MDG 12 l.364 / MSRC 12 l.85), Plat-bord = `moyenne` (MSRC 12 l.111). Géométrie de Pont,
...
- l.489 — `src/data/index.ts:2000` — *  deux natures (`VDM 12 l.646-647`, `VDM 14 l.489`). */
...
- l.137 — `src/data/index.ts:1028` — *  permission du MJ », EDOC 13 l.137 ; lignage Éonir Harioth hors espèces jouables, ADE I 6 l.185).
- l.522 — `src/data/index.ts:1583` — /** Tirage PLURIEL et ALTERNÉ de Mutations au spawn (Marque de Tzeentch, EDOC 13 l.522-524 : « gagne

### ADE I 6 — 1/1 ligne(s) code hors couverture
- l.185 — `src/data/index.ts:1028` — *  permission du MJ », EDOC 13 l.137 ; lignage Éonir Harioth hors espèces jouables, ADE I 6 l.185).
...
- l.113 — `src/state/riverVoyageFlow.ts:889` — // Réparateur de SUBSTITUTION (`MSRC 5 l.113-117`) : le −10 est DÉJÀ fondu dans la valeur jetée,

### NADJ 16 — 1/19 ligne(s) code hors couverture
- l.119 — `src/engine/combat.ts:893` — *  (Middenball NADJ 16 l.119 : « en utilisant les règles habituelles relatives à l'Avantage »).

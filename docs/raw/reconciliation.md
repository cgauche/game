...
**Sens B — Atlas → code (LDB)** : 11 marqueur(s) « (non implémenté) » · 4 chapitre(s) LDB cités par l'Atlas jamais référencés dans le code (avant crédit folio : 20 · 16 crédités par une source folio de `src/data`).
...
### LDB 46 — 11/40 ligne(s) code hors couverture (propriétaire : docs\raw\magie.md)
- l.183 — `src/data/index.ts:1698` — /** Tableau des Vents Tourbillonnants (LDB 46 l.183-190, option `vents-tourbillonnants`) — tirage 1d10
- l.185 — `src/state/combatSlice.ts:3096` — // Focalisation CRITIQUE (LDB 46 l.185-186) : le sort est lançable au prochain Round
- l.188 — `src/data/index.ts:587` — *  (Chamon/Azyr ignorent le métal, Ghur le cuir, LDB 46 l.188). Remplace la devinette par regex sur le nom. */
- l.193 — `src/state/combatFlow.ts:2065` — // Interruption de Focalisation (LDB 46 l.193-194) : Dégâts subis pendant qu'on focalise
- l.194 — `src/engine/ops.ts:634` — /** Marqueur IMPUR de la branche d'ÉCHEC du Test de Calme d'interruption de Focalisation (LDB 46 l.194) :
- l.199 — `src/engine/magic.ts:195` — * « Repousser les Vents » (LDB 46 l.199) : −1 DR aux Tests d'Incantation et de
...
- l.202 — `src/engine/magic.ts:577` — *  Contre-sort (LDB 46 l.202 : « le lanceur tient le rôle attaquant »). Source unique. */
...
### LDB 10 — 6/23 ligne(s) code hors couverture (propriétaire : docs\raw\tests.md)
- l.310 — `src/engine/types.ts:877` — /** Aura magique DÉTECTÉE (Talent Détection d'artefact, LDB 10 l.310-312 : « vous sentez que
- l.364 — `src/engine/types.ts:1410` — /** Distraire (LDB 10 l.364 / AA 13 l.51) : distrait par un adversaire → ne peut gagner AUCUN Avantage
- l.365 — `src/state/combatFlow.ts:4573` — *  Réutilisé par la Chirurgie (infection post-opératoire, LDB 10 l.365) : Chance/Résilience + auto-succès
...
### LDB 12 — 1/31 ligne(s) code hors couverture (propriétaire : docs\raw\tests.md)
...
- `src/data/index.ts:1455` (l.54) — /** Palier de LONGUEUR d'un tarif d'installation (#277 : T2C ch.10 l.54-135 tarife par TYPE de navire à
- `src/data/index.ts:1463` (l.1) — *  Amélioration, l.195-364 ; T2C ch.10 l.1-140) — VERBATIM structuré par paliers de LONGUEUR. `per: '5m'` =
- `src/data/index.ts:1491` (l.85) — *  = `totale` (MDG ch.12 l.364 / T2C ch.10 l.85), Plat-bord = `moyenne` (T2C ch.10 l.111). Géométrie de Pont,
- `src/data/index.ts:1491` (l.111) — *  = `totale` (MDG ch.12 l.364 / T2C ch.10 l.85), Plat-bord = `moyenne` (T2C ch.10 l.111). Géométrie de Pont,

## A2-AUTRES — Lignes appelées par le CODE (autres livres) non pinées par l'Atlas

### EDOC 5 — 12/14 ligne(s) code hors couverture
- l.25 — `src/engine/policy.ts:572` — ref: 'EDOC ch.5 l.25 — MJ décide, valeur maison',
- l.29 — `src/engine/policy.ts:551` — ref: 'EDOC ch.5 l.29',
- l.34 — `src/engine/policy.ts:560` — ref: 'EDOC ch.5 l.34',
...
- l.73 — `src/engine/policy.ts:631` — ref: 'EDOC ch.5 l.73',
...
- l.11 — `src/ui/compendium/codex-exposure-guard.test.ts:165` — 'river-navigation.json': "AUDIT : à exposer -> ticket — table de vent/navigation fluviale (T2C ch.5 l.11-41), consommée par `engine/riverNavigation.ts`, aucune 
...
- l.439 — `src/data/index.ts:1507` — /** Index des ports de la Mer des Griffes (#217, MDG ch.15 l.439-506) — catalogue app-owned éditable au
...
- l.140 — `src/engine/policy.ts:583` — ref: 'EDOC ch.4 l.140-146',
...
- l.185 — `src/data/index.ts:400` — *  permission du MJ », EDOC ch.9 l.137 ; lignage Éonir Harioth hors espèces jouables, ADE1 ch.6 l.185).
...
- l.137 — `src/data/index.ts:400` — *  permission du MJ », EDOC ch.9 l.137 ; lignage Éonir Harioth hors espèces jouables, ADE1 ch.6 l.185).

## A3-AUTRES — Réfs de CODE sans chapitre (`<ABRÉV> l.X`, pas d'unité chapitre à couvrir)

### ADE II — 12 réf(s) sans chapitre
- `src/data/index.ts:1245` (l.62) — /** L'Étoile du Sorcier (ADE2 l.62) : fourchette du 1d10 interne `[min, max]` parmi les variantes
...
- `src/state/combatFlow.ts:267` (l.3900) — // ICI aucune exigence de Compétence pour compter dans l'Équipe (≠ AA/MDG l.3900 ci-dessus) — 3ᵉ courbe,
- `src/state/combatSlice.ts:1788` (l.464) — // DISTINCT du Défaut Arme d'équipe (MDG l.464, `crewedFireWeapon`/`exposedCrew` ci-dessus — headcount
- … +2 autres
...
- **docs\raw\activites.md** L548 — **Implémente :** (non implémenté)
- **docs\raw\activites.md** L781 — **Implémente :** (non implémenté)
- **docs\raw\activites.md** L800 — **Implémente :** (non implémenté)
- **docs\raw\activites.md** L816 — **Implémente :** (non implémenté)
- **docs\raw\activites.md** L839 — **Implémente :** (non implémenté)
- **docs\raw\activites.md** L864 — **Implémente :** (non implémenté)
- **docs\raw\activites.md** L882 — **Implémente :** (non implémenté)
...
Code : AA, ADE I, ADE II, EDOC, MDG, Middenheim, NADAJ, T2C, T3

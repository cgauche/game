...
**Sens A — code → Atlas (LDB)** : 0 chapitre(s) cités par le code & absents de l'Atlas · 1 chapitre(s) couverts avec des lignes non pinées. Réfs folio (`ABBR NN p.X`, #606) côté Atlas : 5 ignorée(s) proprement (ancre absente/ambiguë/hors-chapitre).
**Sens A — code → Atlas (14 autres livres)** : 1 chapitre(s)-livre cités par le code & absents de l'Atlas · 5 chapitre(s)-livre couverts avec des lignes non pinées · 0 réf(s) sans chapitre (non réconciliables par cette mesure).
**Sens B — Atlas → code (LDB)** : 3 marqueur(s) « (non implémenté) » · 3 chapitre(s) LDB cités par l'Atlas jamais référencés dans le code (avant crédit folio : 16 · 13 crédités par une source folio de `src/data`).
...
- l.188 — `src/data/index.ts:608` — *  (Chamon/Azyr ignorent le métal, Ghur le cuir, LDB 46 l.188). Remplace la devinette par regex sur le nom. */
...
- l.85 — `src/data/index.ts:1555` — *  = `totale` (MDG 12 l.364 / MSRC 12 l.85), Plat-bord = `moyenne` (MSRC 12 l.111). Géométrie de Pont,
...
- l.111 — `src/data/index.ts:1555` — *  = `totale` (MDG 12 l.364 / MSRC 12 l.85), Plat-bord = `moyenne` (MSRC 12 l.111). Géométrie de Pont,
...
### EDOC 13 — 3/7 ligne(s) code hors couverture
- l.137 — `src/data/index.ts:417` — *  permission du MJ », EDOC 13 l.137 ; lignage Éonir Harioth hors espèces jouables, ADE I 6 l.185).
- l.230 — `src/engine/ops.ts:981` — *  `rollTable.extraRollsPerStep` (LDB 47 l.13-17, EDOC 13 l.230+270-276). */
- l.522 — `src/data/index.ts:862` — /** Tirage PLURIEL et ALTERNÉ de Mutations au spawn (Marque de Tzeentch, EDOC 13 l.522-524 : « gagne
...
_Avant crédit folio (16)_ : LDB 26 · LDB 27 · LDB 28 · LDB 29 · LDB 30 · LDB 31 · LDB 32 · LDB 33 · LDB 34 · LDB 35 · LDB 38 · LDB 39 · LDB 44 · LDB 50 · LDB 68 · LDB 69

_Crédités par une source folio de `src/data/*.json` (13, donnée référencée sans réf de ligne)_ : LDB 26 · LDB 27 · LDB 28 · LDB 29 · LDB 30 · LDB 31 · LDB 32 · LDB 33 · LDB 34 · LDB 35 · LDB 39 · LDB 50 · LDB 68

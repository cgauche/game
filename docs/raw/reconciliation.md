...
- `src/data/schemas/defs-scenes/scene.ts:89` (l.72) — /** JOUEUR de taverne (`NADJ 04 l.72`) : `gameId` de `tavernGames.json`, mise de DÉPART en sous. */
- `src/state/scene.ts:114` (l.72) — *  Le patron est AUTHORÉ dans la source, pas inventé : `NADJ 04 l.72` — « Elle jouera une partie de
...
- `src/data/index.ts:790` (l.5) — *  Portée sur `gnomes` (`NADJ 14 l.5`, règle `creation-gnome-jouable`). */
...
- l.85 — `src/data/index.ts:2384` — *  = `totale` (MDG 12 l.364 / MSRC 12 l.85), Plat-bord = `moyenne` (MSRC 12 l.111). Géométrie de Pont,
...
- l.111 — `src/data/index.ts:2384` — *  = `totale` (MDG 12 l.364 / MSRC 12 l.85), Plat-bord = `moyenne` (MSRC 12 l.111). Géométrie de Pont,
...
- l.489 — `src/data/index.ts:2005` — *  deux natures (`VDM 12 l.646-647`, `VDM 14 l.489`). */
...
- l.137 — `src/data/index.ts:1033` — *  permission du MJ », EDOC 13 l.137 ; lignage Éonir Harioth hors espèces jouables, ADE I 6 l.185).
- l.522 — `src/data/index.ts:1588` — /** Tirage PLURIEL et ALTERNÉ de Mutations au spawn (Marque de Tzeentch, EDOC 13 l.522-524 : « gagne

### ADE I 6 — 1/1 ligne(s) code hors couverture
- l.185 — `src/data/index.ts:1033` — *  permission du MJ », EDOC 13 l.137 ; lignage Éonir Harioth hors espèces jouables, ADE I 6 l.185).

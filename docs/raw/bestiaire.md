...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `LDB 14` (l.142-165) → `GrappleModal`, `combat-deux-armes`, `areGrappling`, `main-secondaire`, `setGrapple`, `scatter`, `empetre`, `grappleTierMod`, `tirer-dans-le-tas`, `grappleEnvMod`, +40 — `src/data/grapple.json`, `src/data/index.ts`, `src/data/regles.json`, `src/data/schemas/defs/grapple.ts`, `src/data/schemas/defs/sizes.ts`, `src/engine/combat.ts`, +25 fichiers

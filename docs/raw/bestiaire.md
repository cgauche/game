...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `LDB 76` (l.19-45) → `STANDARD_OPTIONALS`, `aaTableFor`, `resolveAACritical`, `SceneEntity`, `criticalTableFor`, `SpawnExtras`, `creatureToCombatant`, `critTableKeyFor`, `rollCritical`, `TraitData` — `src/data/criticals.ts`, `src/data/index.ts`, `src/engine/aaCritical.ts`, `src/engine/critical.ts`, `src/state/scene.ts`, `src/state/spawn.ts`, +1 fichiers
...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `LDB 76` (l.28-35) → `STANDARD_OPTIONALS`, `aaTableFor`, `resolveAACritical`, `SceneEntity`, `criticalTableFor`, `SpawnExtras`, `creatureToCombatant`, `critTableKeyFor`, `rollCritical`, `TraitData` — `src/data/criticals.ts`, `src/data/index.ts`, `src/engine/aaCritical.ts`, `src/engine/critical.ts`, `src/state/scene.ts`, `src/state/spawn.ts`, +1 fichiers
...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `LDB 85` (l.343-406) → `creatureWeapon`, `cannotStopOn`, `agressifEnvers`, `markAttacked`, `weaponFromTrait`, `EnemyTurnInput`, `woundsForSize`, `displaceSmaller`, `MoveEnv`, `availableAttacks`, +37 — `src/data/index.ts`, `src/data/maneuvers.json`, `src/data/regles.json`, `src/data/traits.json`, `src/engine/creatureEquip.ts`, `src/engine/engagement.ts`, +8 fichiers
...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `LDB 85` (l.276-277, l.339-340) → `creatureWeapon`, `StatblockEditor`, `woundsForSize`, `resizeBySteps`, `resolvePsychAI`, `creatureToCombatant`, `taille-modificateurs-en-combat`, `sonne`, `souffle-feu`, `souffle-froid`, +25 — `src/data/index.ts`, `src/data/maneuvers.json`, `src/data/regles.json`, `src/data/traits.json`, `src/engine/creatureEquip.ts`, `src/engine/size.ts`, +5 fichiers
...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `LDB 14` (l.142-165) → `GrappleModal`, `areGrappling`, `setGrapple`, `scatter`, `combat-deux-armes`, `empetre`, `grappleTierMod`, `main-secondaire`, `effectiveSize`, `grappleEnvMod`, +45 — `src/data/grapple.json`, `src/data/index.ts`, `src/data/regles.json`, `src/data/reglesOptionnelles.json`, `src/data/schemas/defs/grapple.ts`, `src/data/schemas/defs/sizes.ts`, +26 fichiers
- `LDB 85` (l.357-387) → `cannotStopOn`, `agressifEnvers`, `markAttacked`, `weaponFromTrait`, `EnemyTurnInput`, `woundsForSize`, `displaceSmaller`, `MoveEnv`, `availableAttacks`, `taille-modificateurs-en-combat`, +11 — `src/data/index.ts`, `src/data/regles.json`, `src/data/traits.json`, `src/engine/creatureEquip.ts`, `src/engine/engagement.ts`, `src/engine/psychology.ts`, +7 fichiers
...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `LDB 77` (l.108) → `CustomStatblock`, `randomizeChars`, `Possession`, `SceneEntity`, `skillInstance`, `SpawnExtras`, `StatblockEditor`, `creatureToCombatant`, `statblockToCombatant`, `prejuge`, +4 — `src/data/creatures.json`, `src/data/reglesOptionnelles.json`, `src/engine/possession.ts`, `src/engine/statblock.ts`, `src/state/scene.ts`, `src/state/spawn.ts`, +2 fichiers

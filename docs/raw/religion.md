...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `LDB 40` (l.45-50) → `discreetPrayerDifficulty`, `peche`, `CastModal`, `Effect`, `prayerWrathTriggered`, `GameOp`, `CastPenalty`, `OPTIONAL_RULES`, `FLOWS`, `colere-visions-sacrees`, +38 — `src/data/characteristics.json`, `src/data/miscast.json`, `src/engine/magic.ts`, `src/engine/miscast.ts`, `src/engine/ops.ts`, `src/engine/policy.ts`, +10 fichiers

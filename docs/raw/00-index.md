...
- **`node scripts/raw/check-source-tables.mjs`** (#1384) — les TABLES du `Source/` que leur FORME rend inadressables, en cinq familles : `br-litteral` (un `<br>` littéral dans une cellule), `span-colle` (marqueur `data-folio` collé à une ligne de table), `donnee-en-tete` (continuation après saut de page dont les « en-têtes » sont une fourchette), `banniere-suspecte` (bandeau de titre que le parseur n'absorbe pas) et `cle-de-ligne-ambigue` (clé de ligne partagée par deux tables d'une même section). **Stock NOMINATIF DÉCROISSANT** `scripts/raw/source-tables-stock.json` (une entrée par site, clé `famille :: fichier :: réf :: occurrence`) : un site hors stock ET une entrée sans site sont rouges. Le geste de réparation est le tableau « défaut de table → geste » de [`../ajouter-un-livre-source.md`](../ajouter-un-livre-source.md) §7 ; le stock se régénère par `--ecrire-stock`.
- **[`reanchor.md`](reanchor.md)** (`node scripts/raw/reanchor.mjs` ; `--apply` verbatim + `--remap` synthèse) — ré-ancre les réfs `l.X` contre la Source Marker : citations « … » par **match exact**, réfs de synthèse par **diff `git HEAD`↔arbre** (one-shot à relancer après chaque ré-extraction, avant de committer la Source). **Seuils : 🔧 = 0 · 🟡 = 0 · ❌ sous cliquet NOMINATIF par site (`reanchor-low-stock.json`).** Voir l'**[épreuve du 2026-06-22](epreuve-2026-06-22.md)**.

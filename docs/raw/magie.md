...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `LDB 48` (l.398) → `clarte-d-esprit`, `fauche-demon`, `sonne`, `castContextMods`, `aveugle`, `lumiere-de-guerison`, `pensee-rapide`, `brise`, `arme-enchantee`, `creuset-de-chamon` — `src/data/spells.json`, `src/engine/domainAttributes.ts`, `src/state/combatFlow.ts`

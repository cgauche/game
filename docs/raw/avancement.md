...
- `LDB 9` (l.44) → `CibleDeType`, `specEntrySchema`, `redaction`, `buySkillAdvance`, `athletisme`, `estSpecialisable`, `wildcardSpecs`, `specRef` ⚠sans-appelant, `buildAdvancementView`, `combatBaseValue`, +10 — `src/data/index.ts`, `src/data/schemas/defs/skills.ts`, `src/data/schemas/grammaire/ref.ts` ⚠hors-app, `src/data/skills.json`, `src/engine/activities.ts`, `src/engine/advancement.ts`, +10 fichiers

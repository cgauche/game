...
- `LDB 9` (l.44) → `specEntrySchema`, `redaction`, `buySkillAdvance`, `athletisme`, `wildcardSpecs`, `designateSpec`, `buildAdvancementView`, `combatBaseValue`, `weaponUnmastered`, `SENTINELLE_CHOIX`, +13 — `src/data/index.ts`, `src/data/schemas/common.ts`, `src/data/schemas/defs/skills.ts`, `src/data/skills.json`, `src/engine/activities.ts`, `src/engine/advancement.ts`, +12 fichiers

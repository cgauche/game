...
- `LDB 9` (l.44) → `specEntrySchema`, `redaction`, `buySkillAdvance`, `athletisme`, `wildcardSpecs`, `buildAdvancementView`, `combatBaseValue`, `weaponUnmastered`, `createHero`, `SpecsField`, +7 — `src/data/index.ts`, `src/data/schemas/defs/skills.ts`, `src/data/skills.json`, `src/engine/activities.ts`, `src/engine/advancement.ts`, `src/engine/careerSlots.ts`, +9 fichiers

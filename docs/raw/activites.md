...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `EDOC 8` (l.129-180) → `plein-air`, `PosteCardinality`, `printemps`, `activityAsPoste`, `approvisionnement`, `ete`, `gatherInnInfo`, `recueillir-informations`, `EncounterCategory`, `automne`, +24 — `src/data/activities.json`, `src/data/reglesOptionnelles.json`, `src/data/voyage-stakes.json`, `src/data/weather.json`, `src/engine/activities.ts`, `src/engine/travelEncounter.ts`, +8 fichiers
...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `ACE 12` (l.23-42) → `InterludeHeroState`, `BankDeposit`, `OutcomeBand`, `matchOutcomes`, `combatSkillPick`, `weaponUnmastered`, `itemFromTrappingById`, `weaponGroupSkillMode`, `ActivityDef`, `penitence`, +15 — `src/data/activities.json`, `src/data/index.ts`, `src/engine/activities.ts`, `src/engine/combat.ts`, `src/engine/items.ts`, `src/engine/types.ts`, +2 fichiers

...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `T2C 16` (l.149-160) → `schema`, `Disease`, `resolveInlineFlowTest`, `combatTestPenalty`, `crampes-abdominales`, `testStatePenalty`, `firingOwnTestFailed`, `EffectTrigger`, `tickDisease`, `routeTriggeredTest`, +6 — `src/data/index.ts`, `src/data/maladies.json`, `src/data/schemas/defs/symptoms.ts`, `src/data/symptoms.json`, `src/engine/conditions.ts`, `src/engine/disease.ts`, +7 fichiers

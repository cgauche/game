...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `MSRC 16` (l.121-144) → `onTickSchema`, `doc`, `DiseaseDef`, `OPS_FIELDS`, `Disease`, `diseaseTestModLines`, `snapshotInfectionResidual`, `crampes-abdominales`, `resolveInlineFlowTest`, `combatTestPenaltyParts`, +13 — `src/data/index.ts`, `src/data/maladies.json`, `src/data/schemas/defs/maladies.ts`, `src/data/schemas/defs/symptoms.ts`, `src/data/symptoms.json`, `src/engine/conditions.ts`, +9 fichiers

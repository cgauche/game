...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `MSRC 16` (l.149-160) → `schema`, `Disease`, `resolveInlineFlowTest`, `inconscient`, `combatTestPenaltyParts`, `firingOwnTestFailed`, `testStatePenaltyParts`, `tickDisease`, `EffectTrigger`, `vers-du-reik`, +7 — `src/data/index.ts`, `src/data/maladies.json`, `src/data/schemas/defs/symptoms.ts`, `src/data/symptoms.json`, `src/engine/conditions.ts`, `src/engine/disease.ts`, +7 fichiers

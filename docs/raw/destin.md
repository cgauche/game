...
**Implémente :** _(généré — `npm run raw:implemente`)_
- `LDB 17` (l.41) → `restoreFortune`, `fateSaveOrDie`, `Effect`, `GameOp`, `GameState`, `endSession`, `EFFECT_HANDLERS`, `finalizeHeroDeath`, `Combatant`, `createCombatSlice`, +1 — `src/engine/fortune.ts`, `src/engine/ops.ts`, `src/engine/types.ts`, `src/state/combatEffects.ts`, `src/state/combatFlow.ts`, `src/state/combatSlice.ts`, +3 fichiers

...
Compléter le Marchand : **Marchandage** (négocier le prix), **Évaluation** (identifier la qualité cachée + estimer la valeur), **Réparation** d'armure, et le **modèle d'objet non-identifié / magique** (qualités cachées, révélées par l'Évaluation, avec le skin légendaire comme indice). Tout RAW (LDB 60 + 63 ; qualités magiques = ADE II), rien d'inventé.

## Décisions (brainstorming 2026-06-08)

- **Objet magique ≠ nouveau moteur** : un objet magique/légendaire = un objet avec des **qualités EN PLUS** (normales comme Dévastateur, ou magiques d'ADE II). Le **registre de qualités existant les applique déjà** au combat — on n'ajoute QUE le fait de les **cacher/révéler**.
...
- **Qualités magiques ADE II** = contenu (defs du registre), extraites+citées au plan ; 1-2 en v1 pour prouver le pipeline, le reste = passes ultérieures.
...
- Source : **ADE II** (« règles de création d'objets magiques » — extraites+citées au plan). Ajoutées comme entrées du registre (subType `Magique`), branchées via le **dispatch existant** (comme Dévastateur). **1-2 en v1** pour prouver le pipeline ; le reste = contenu incrémental.
- Un objet magique de démo = un objet `identified:false` + qualité(s) (normale type Dévastateur et/ou magique ADE II) + skin, posé en loot authored.
...
- registre : 1-2 qualités magiques ADE II chargées + appliquées (golden-combat couvre l'iso-comportement).
...
- Jeu complet de qualités magiques ADE II (au-delà des 1-2 démos) = contenu ultérieur.
...
- **Pas de placeholder** : signatures (`identified`, `appraiseValueEstimate`, `bargainOutcome`, `repairCost`, actions store) ; RAW **cité** (LDB 60 l.10/12/22, LDB 63 l.97-98) ; qualités magiques = ADE II, extraites au plan (source identifiée).

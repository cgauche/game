 ...
 * d'import → moteur pur). Affichage = NOMS CANON FR (décision utilisateur) : CO (couronne d'or) /
 * pa (pistole d'argent) / sc (sou de cuivre). `formatMoney` est la SOURCE UNIQUE d'affichage (le HUD
 * et le log `giveMoney` la consomment — fix transverse, cf. « Transverse coin-label » ci-dessous). */
 ...
 *  pa=pistole d'argent, sc=sou de cuivre (cf. LDB 57). SOURCE UNIQUE d'affichage de la monnaie. */
...
## Transverse coin-label — noms canon FR (DÉCISION PRISE : option b)
Décision utilisateur : **noms canon FR**. `formatMoney` (Task 1) = **CO / pa / sc** (couronne d'or / pistole d'argent / sou de cuivre, LDB 57). `formatMoney` est la **source unique** — deux sites existants à rebrancher dessus :
- **HUD Bourse** `src/ui/CampaignView.tsx` (~l.62-66) : remplacer l'inline `… CO · … SC · … PA` par `<span className="coins">{formatMoney(money)}</span>` (import `formatMoney`). **Fait dans Task 5** (CampaignView déjà touché).
- **Log `giveMoney`** `src/state/combatFlow.ts` (~l.160-171) : les libellés `${e.gold} CO` / `SC` / `PA` du log → `CO`/`pa`/`sc` (garder le ± des deltas). **Petit edit isolé (fichier chaud)** — Step dédié dans Task 1.

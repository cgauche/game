...
**Conséquence :** `scripts/frenchy/term-map.json` (bâti sur Khaos) marque à tort `gap-missing-in-ours` des talents qui EXISTENT sous un autre nom FR. Toujours re-vérifier via VO + l'effet dans la source FR autorisée avant de conclure « absent ».

**Avant de dire « absent », vérifier la DONNÉE RÉELLE (`src/data/creatures.json`), PAS le term-map ni le registre de defs (`engine/traits/defs/`).** Le nettoyage `1768092` avait déjà correctement remappé la quasi-totalité des libellés frenchy (talents ET traits ET compétences) vers leur nom officiel : mes « talents/traits perdus » étaient ~tous des FAUX POSITIFS (libellé officiel ≠ libellé frenchy normalisé). Ex : trait `Redoutable 2` (ZI/Grim) est porté par ~30 créatures mais n'a PAS de def dans `engine/traits/defs/` (trait-label sans effet codé) → chercher la def renvoie « absent » à tort. Le user a corrigé 3× : « Fusilier est dans AA » → Artilleur, « Regarde la VO » → Artisan=Maître artisan, « j'avais rajouté les trait ZI » → Redoutable déjà sur les créatures. Vraies lacunes trouvées = SEULEMENT côté talents (Fusilier/Artilleur, Vigilance, Effraction, Pas de Côté, Détection d'artefact, Artisan) ; traits/compétences = 0 à restaurer. Prolonge [[game-frenchy-bzh-creatures]] + [[feedback-source-user-claims]].

...
- **« de qualité »** → une **RÈGLE définie** (LDB ch.60 « Fabrication » p.293) : une « Possession de Qualité » = un objet portant plus d'**Atouts d'objet** que de Défauts (chaque Atout double prix + baisse dispo). Le « de qualité » générique des possessions = l'Atout **`Raffiné`**. ⚠ Les 8 Atouts/Défauts de Fabrication (Raffiné/Léger/Pratique/Solide/Bâclé/Laid/Peu-Fiable/Volumineux) SONT au registre `qualities.json` (sourcés LDB p.286), et une `TrappingRef` de dotation SAIT attacher un Atout à l'objet octroyé : `trappingRefSchema` porte `qualities?` (+ `qualityChoice?`) sur la branche `{id}` (`src/data/schemas/common.ts`), matérialisé par `buildInventory` (`src/engine/items.ts`) qui append via `qualityInstance` — ce qui PRÉSERVE le `value` d'un « Solide 3 » (un merge par id nu le perdrait). NE PAS droper « de qualité » comme saveur.

...
**État 2026-09-04 nuit (main = c2940eaec)** : B3-2b-b POSÉ (écran « Stations à bord », primitives `OptionChooser refus`/`StakeRule` nom de la fiche/case vide sans mot) — la série B3-2 est entière. Doctrine du soir (dés) : TOUT dé passe par la porte unique (« comme Foundry »), aucune taxonomie n'exempte → #1508 = faire passer chaque dé hors porte PAR la porte (juge de design d'abord). Suite : juge de design « PostesRoster par poste » (maquette à l'utilisateur, 3 écrans) → #1508 → B2e météo → #1681/#1684 → famille table → #1620 (iii) → L1a/L1c → #1654 → portes → gardes → #1437 + #918 → réitération #1673.

**État 2026-09-05 (main = 067c55758)** : #1689 T1 POSÉ (roster par poste, primitive unique, 4 écrans, règle « épinglé seul » partout) ; #1508 T1 (socle « étape de dé » + garde sœur) codeur EN COURS ; suite : #1508 T2-T6 entrelacés avec les fusions UI #1689 (T2 refus, T3 title natif, …) → B2e → #1681/#1684 → famille table → #1620 (iii) → L1a/L1c → #1654 → portes → gardes → #1437 + #918 → réitération #1673.

**État 2026-09-06 (main = c9ad5a200)** : #1508 T2 POSÉ (99c5da297) — canal op → applier ouvert, la chute passe par la porte sur ses routes JOUABLES, stock (X) 45 entrées / 305 → 303 ; le train a coûté HUIT passes de juge de diff et TROIS recettes navigateur (leçon : [[game-cascade-trois-pilotes-a-verrouiller]] — un socle de cascade se prouve sur les trois pilotes ET depuis le geste joueur ; les contrats qui appellent l'applier à la main ne prouvent rien) ; formulaire `fall` de l'éditeur, scénario de test « Chute du gréement », helpers `__wfrp.shipCrit`/`station` ; restes de recette → #1696 ; JUGE-VISION soldant e1f606503/9cb065cf5. Suite : #1689 T2 (raison de refus → `GatedAction`, codeur lancé) puis #1508 T3-T6 entrelacés. File d'intégration du jour : C3 (#1388) → T2 → L3b (#1679). Arbre principal : commit local non poussé 599979762 (#1680, base vieille de 59 commits) bloque le ff — signalé, pas touché.

**État 2026-09-06 après-midi (main = 6a30233aa)** : #1689 T2 POSÉ (54d228a46) — la raison d'un refus n'a qu'une forme (`GatedAction`), 53 sites joueur migrés (62 → 0 par entrées, atelier gelé à 21, 13 exemptions AU SITE), `OptionChooser` `actions` = instance de `grid`, `seg` → T4 ; trois passes de juge + deux recettes ; `GatedAction` a CONVERGÉ avec #1599 (même invariant « jamais de `disabled` natif », amont gardé) ; restes → #1698. Codeur #1508 T3 lancé (sauvegardes à seuil du héros, `brief-1508-t3.md`). Suite : #1508 T3 → #1689 T3 (title natif, 144 sites) → #1508 T4-T6 → #1689 T4-T10.

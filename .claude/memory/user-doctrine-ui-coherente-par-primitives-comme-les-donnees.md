...
**Verbatim 3 (2026-09-04 nuit, après la découverte de `ShipCrewByRole` — un second roster de rôles d'équipage, bâti sur `AssignRow`, avec une inférence différente de celle de `ShipRolesPanel`, manqué par le juge de design)** : « Tu comprends maintenant pourquoi j'essaye de fusionner et d'asseinir le projet de toutes ses variantes d'interface »

**Cas fondateur** : concept « roster de postes » = 3 formes (`PostesRoster` héros-first, `ShipCrewByRole` rôle-first sur `AssignRow`, `AssignRow` nu dans `MassBattleView`) et 2 régimes d'inférence pour `shipRole` (`defaultCrewRole` vs `shipDefaultRoles`). Une primitive unique aurait rendu la 2ᵉ forme impossible et la question d'inférence posée UNE fois. Règle qui en découle : AVANT tout design d'écran, un INVENTAIRE PAR CONCEPT D'INTERFACE (grep des primitives voisines : `AssignRow`, `PortraitPicker`, `PanneauParametre`, `OptionChooser`… et de leurs composeurs) — le juge de design d'un écran commence par « quelles formes existantes portent déjà ce concept ? » ; la vague « inventaire UI par concept » (patron #1673) est le geste structurel.

...
**How to apply:** pour valider `HEAD` en isolation → `git worktree add --detach <tmp> HEAD` puis **`npm install` PROPRE dans le worktree** (son propre node_modules, jamais de junction/symlink vers le principal). Worktree pour agents = même règle ([[game-agents-worktree-isolation-shared-branch]]) : un vrai install, lent mais sûr. Nettoyer avec `git worktree remove`.

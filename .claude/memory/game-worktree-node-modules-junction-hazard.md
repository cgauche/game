...
**Ce qui suit la jonction, et ce qui ne la suit pas** (mesuré en dépôt jetable le 2026-09-03) : le geste destructeur de l'incident est le `rmdir` — il traverse le point de reparse et vide la cible. `git clean -fdx` dans le worktree RETIRE le point de reparse et laisse le `node_modules` partagé intact avec tout son contenu. C'est ce qui permet au garde de se taire sur `clean` dans un worktree à répertoire prouvé sans rouvrir cet incident-là.

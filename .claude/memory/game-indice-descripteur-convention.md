...
- **Défaut UNIVERSEL = 1** : le moteur fait partout `r.indice ?? 1` — trois sites dans `engine/qualities/dispatch.ts` (`return r ? r.indice ?? 1 : undefined`, la variante `: 0`, et le seuil `(r.indice ?? 1) >= 2`). Donc « un seul → Indice 1 » n'est pas propre à une entité — c'est le défaut global.

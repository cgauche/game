...
**#1472 + T3 (prochain, arbitré GO)** — la prescription COMPLÈTE vit dans le rendu du juge de design V-CODEX (transcript session 2026-08-29) et l'essentiel au commentaire #1472 du 29/08 : classes C (~16 catégories qui DROPPENT desc/source — les surfacer), E (gods `src()` manquant, structures `fact('Source')` doublon), helper `depuisEnveloppe` DÉJÀ posé (registry.ts:208, ne pose source que si `src()` non-null). Preuve exigée : liste nominative de deltas GELÉE dans `registry-enveloppe.test.ts` (empreinte CLES item-par-item), cliquet des 39 muettes DÉCROISSANT, recette navigateur + juge vision (l'écran change). ⚠ Classe H (10 singletons, 4 ids camel⇄kebab) = arbitrage utilisateur SÉPARÉ, PAS tranché. Arbitrage T3 verbatim au commentaire #1472.

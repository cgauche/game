...
**How to apply :** l'orchestrateur orchestre, revoit et vérifie ; toute édition de code passe par un agent dont le `model` est EXPLICITE au dispatch. Sur toute migration : sweep de la table « Primitives partagées » et grep de l'existant AVANT tout module neuf, primitives cibles nommées avant le code, extension du général plutôt que duplication, commentaires citant le RAW re-vérifiés au `Source/`, et travail FINI — pas de refactor à moitié.

...
1bis. **UN mot-clef PAR ticket** : `corrige #1709 #1708` ne ferme que #1709 — ni la porte de commit ni le job `fermetures` ne lisent un `#N` nu en seconde position (mesuré 2026-09-12 : la porte n'a exigé que `1709.md`, le job n'a fermé que #1709, #1708 est resté ouvert et a demandé un commit rectificatif `corrige #1708`). Forme juste : `corrige #1709, corrige #1708`. La grammaire vit en UN endroit, `scripts/guards/lib/fermetures.mjs` (`numerosFermes`), consommée par la porte ET le job — avant, `fix #N`/`close #N` passaient la porte sans fermer à la publication, et `fixe #N` (prose française, `df1507439`) aurait fermé #939.
...
3. Le PALIER se MESURE sur l'histoire (`scripts/guards/lib/revuePalier.mjs`, `mesureDuPalier`) : commits touchant `src`/`scripts` depuis la tête de fenêtre de la dernière revue de HEAD. À 10, aucune fermeture sans une revue adversariale du CUMUL, écrite DIRECTEMENT sous son nom d'archive `.claude/soldes/revue-palier-<date>-<base>.md` et stagée par le commit — sa fenêtre `<base>..<tête>` part là où la précédente s'arrête, et le palier repart dès ce commit.

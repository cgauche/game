...
description: "Système de vision / brouillard de guerre (LoS, lumière, IA réciproque, portes) — architecture + invariants perf ; le culling brouillard vit désormais dans fogVeilObjs/CulledScene (post game-iso-depth-perface-hard)."
metadata: 
  node_type: memory
  type: project
...
Système de vision/brouillard de guerre (branche `feat/wfrp4-rpg-foundation`). Demande user : « si pas en vue, on ne voit rien — même le décor ; pas à travers les murs ». Tout data-driven (éditable au Codex).
...
**État/rendu** : `GameState.explored` (persistant, hors reset scene) + `visionState.computeStateVisible` (union héros vivants/groupe) ; `visible` DÉRIVÉ au memo d'`IsoStage` (stable pendant la marche, cf. [[game-isostage-walk-rerender-perf]]) + `markExplored`. `src/gameIso/FogLayer.tsx` : overlay 3 états (inconnu noir/exploré grisé/vu net), bords adoucis. Culling des créatures hors-vue ; ghost conservé (vu du groupe mais hors LdV du tireur actif). Échelle 1 case = 2 m.
...
**⚠️ Évolution architecturale constatée (vérifiée au code, 2026-07-05)** : la séparation viewport/culling vit désormais dans `stage/CulledScene.tsx` + `fogVeilObjs` (`builders/floors.ts`), plus dans une fonction FogLayer monolithique. Un bug de profondeur y a été trouvé et corrigé séparément (cf. [[game-iso-depth-perface-hard]] : le split visible/!visible de `CulledScene` écrasait l'ordre de profondeur — un mur en vue passait par-dessus une rampe hors-vue — fixé par un voile PAR case entrelacé). Les invariants perf ci-dessus (grille O(1), Set d'arêtes, culling viewport) restent VRAIS ; c'est leur implémentation qui a bougé de fichier.

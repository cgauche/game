...
Pourquoi : l'ancien modèle z-dominant (tout z=N après tout z=N−1) rendait un élément VERTICAL multi-niveaux (mur/escalier) intriable — ancré bas il passait sous le sol z1 (courtine plate, porte invisible), ancré haut il passait DEVANT la cour (le mur avalait la cour en edge-on). Le z-secondaire imbrique par vraie position écran. Sûr car AUCUN porte-à-faux LATÉRAL dans le contenu — « porte-à-faux » = décalage colonne≠colonne ; le surplomb VERTICAL aligné colonne (pont au-dessus d'une rue : z1 sur z0 vide de la MÊME case) est supporté et rendu (piliers auto `overhang-pillar` de `builders/floors.ts`, scène-témoin `pont-vitrine`). ⚠ C'est une hypothèse de CONTENU, gardée par AUCUN code (aucune validation de carte ne l'empêche — audit 2026-08-09) ; la réf `rampartTilesAbove` citée à l'origine n'existe plus dans `src/` (0 match).

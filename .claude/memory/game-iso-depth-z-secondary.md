...
LEÇON (coûteuse) : un bug de RENDU se vérifie OBLIGATOIREMENT dans **les 2 projections (`camEdge` true/false) × les 4 rotations (`camRot`)** — le losange paraissait correct pendant que l'edge-on était cassé. Ne JAMAIS dire « corrigé » sur un seul angle. Cf. [[game-murs-aretes-systeme]], [[game-vision-fog-of-war]], [[game-opera-nadj-multiniveau-program]], [[feedback-orchestrator-verify-delete-redo]].

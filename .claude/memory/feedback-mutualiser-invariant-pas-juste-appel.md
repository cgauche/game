...
**How to apply:** mesurer la duplication par « un appelant peut-il violer l'invariant ? », PAS par le nombre de lignes. Si oui → extraire une primitive ET retirer le footgun **à la source** (ici : `applyCriticalToTarget` ne re-tire plus rien, son param `location` EST la loc résolue ; `critWoundLocation(rng, bodyShape?, override?)` = source unique de LDB 18 l.53). Le double tirage devient structurellement impossible. Committé 7b5cff15. Cf. [[credo-exemples-calibrants]] [[feedback-effet-existant-general-parametrable]] .

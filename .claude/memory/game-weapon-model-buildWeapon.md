...
**Vocabulaire ops** : `grantWeapon` (arme dans set dédié) · `grantNaturalWeapon` (attaque additionnelle) · `augmentWeapon` (enchante l'arme TENUE, ne construit pas). Champs INTERNES gardés (impl, hors vocabulaire éditable au Codex) : `ActiveEffect.conjuredSet`, module `conjuredWeapons.ts`, fichier `conjure-weapon.test.ts`. **L'enchantement vit sur l'OBJET** : `ItemInstance.enchants?: WeaponEnchant[]` (`src/engine/types.ts`), replié dans l'arme dérivée par `recomputeLoadout`/`applyEnchants` ; l'`ActiveEffect` n'en porte que la temporisation (`enchantRef: { itemUid, enchantId }`) pour le retrait ciblé à l'expiration — jamais l'enchantement lui-même.

Prolonge [[game-weapon-registry]] et [[game-items-unified-vocabulary]].

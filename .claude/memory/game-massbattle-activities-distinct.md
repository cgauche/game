...
Ce qui **se FOND** dans le système d'Activité unique (décision utilisateur 2026-07-05, cf. [[game-activites-unification-chantier]]) :
...
Faux obstacles écartés (B les avait pris pour des incompatibilités) : paliers vs bandes = isomorphes ; Test combiné / assistance multi-PJ / prérequis `requires`/`grantsFlag` = capacités GÉNÉRIQUES d'`ActivityDef`, pas un schéma à part.

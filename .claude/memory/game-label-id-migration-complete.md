...
RESTE (hors périmètre, PAS à moi) : une session PARALLÈLE a édité combat/manœuvres pendant la migration (op `grantFreeAttack`, retrait `battleTentacle`, `talents en données`) → `tentacle-attack.test`/`trample.test`/`GameOpEditor.tsx (grantFreeAttack)` rouges, à elle de finir (j'ai assumé le sweep en co-éditant au niveau ligne). Régression PRÉ-EXISTANTE repérée (antérieure, hors migration) : pénalités de port d'armure `en-discretion` ont perdu leur `-N%` (migration qualités antérieure) — `parseWearPenalty` ne les relit plus, à traiter séparément.

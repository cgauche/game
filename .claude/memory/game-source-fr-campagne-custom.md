...
`Game/Source/` ne contient pas que le Livre de base + Archives de l'Empire : aussi les **livres de campagne en français** (`Warhammer v4 - 1.0 L'ennemi dans l'Ombre` + Compagnon, `2.0 Mort sur le Reik` + Compagnon extrait 19 chapitres, Boîte d'Initiation, etc.). Le CLAUDE.md § Sources VF liste les livres à périmètre documenté (LDB, ADE I/II, EDO/EDOC, Middenheim, AA, ZI, MDG, ACE).
...
**`build:data` a été SUPPRIMÉ** (il régénérait `src/data/*.json` depuis `Source/all-data.json` et écrasait les données curées). `src/data/*.json` est désormais la **source unique app-owned, éditée DIRECTEMENT** (Compendium, cf. [[game-data-driven-architecture]]). `Source/all-data.json` reste une ancienne extraction dormante (LDB/ADE1/ADE2+EDO/Middenheim/EDOC) — plus consommée.

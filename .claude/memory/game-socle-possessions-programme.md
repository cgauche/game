...
⚠ **Gate de PALIER** : le palier se MESURE sur l'histoire (`scripts/guards/lib/revuePalier.mjs`, `mesureDuPalier` — #1679 L2) : commits de SUBSTANCE (`src`/`scripts`, fermeture ou pas) depuis la tête de fenêtre de la dernière revue de HEAD, plus le commit en cours s'il en touche. À **10** (`PALIER`), `scripts/hooks/solde-ticket-guard.mjs` exige une revue adversariale du CUMUL, écrite DIRECTEMENT sous son nom d'archive `.claude/soldes/revue-palier-<date>-<base>.md` et STAGÉE (ligne `verdict: CONFIRMÉ|PARTIEL|RÉFUTÉ` + ≥80 car. de synthèse + date du jour en 1re ligne + fenêtre `<base>..<tête>` qui ENCHAÎNE sur la précédente). Pour savoir si le gate mord : `git rev-list --count <tête>..HEAD -- src scripts`.

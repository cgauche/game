...
- **B1b (e35c420)** : `appearance.features: string[]` (clés catalogue) → traits ADDITIFS posables sur N'IMPORTE QUEL PNJ (race→def→instance dans composeRig). Pipeline : rigFieldsFrom/riggedAppearance/entityRigProfile/spawnEnemy/pickBackend. Éditeur : picker multi « Traits du corps » (MonsterPartsFields → CodexEdit créature + Inspector scène). `elementsOf` ré-ajouté. Preuve testée (`elements.test.ts`) : Humain porte les oreilles d'Elfe (même art partagé). Byte-identique.

- **Catalogue → REGISTRE** (exigence user « ajouter un élément = déposer un fichier ») : `APPEARANCE_ELEMENTS` n'est plus un objet inline mais un registre auto-chargé `parts/elements/defs/<key>.ts` (gen-registry → `ELEMENT_DEFS`), 15 traits = 15 fichiers ; `parts/elements/index.ts` dérive le catalogue ; type `ElementOverlay` (superset scale+replace/behind). Byte-identique. (Le dossier `parts/elements.ts` est devenu `parts/elements/`.)
...
- **Bascule frenchy** (WIP user, non commité) : bipèdes Skaven/Gor/Démon → bonne espèce ; nuées → gabarit `swarm` (trait Nuée). Engine swarm committé (9fcd9ad).
...
**Défaut de tenue (recadrage user 2026-06-14)** : « Nu » EST le bon défaut GLOBAL (corps nu pour créatures vraiment sans habit). « Tous les PNJs à poil » = les PNJs n'avaient pas d'apparence paramétrée → fix = PARAMÉTRER, pas changer le défaut. Les 4 espèces civilisées (Humain/Nain/Halfling/Haut-Elfe/Elfe sylvain) étaient les SEULES sans `race.tenue` (les 14 autres races en ont une) → ajout `race.tenue` habillé : Humain/Halfling/Elfe→`Bourgeois`, Nain→`Artisan`. Chaîne : `c.career ?? cd?.tenue ?? perso?.tenue ?? race.tenue ?? 'Nu'` (carrière/record/scène priment toujours). Habille d'un coup les humains officiels (Bella la Noire/Pol Dankels/Eusapia + génériques) ET les ~160 PNJ frenchy.bzh (espèce Humain) sans toucher au WIP frenchy. Tests `enemyProfile.test.ts`/`creatures.test.ts` migrés Nu→Bourgeois. **PAS committé** : la fix change les goldens `creature-render`/`creature-combat-render` qui sont ENTANGLÉS au WIP frenchy (index.ts merge `frenchy-creatures.json` non commité) → doit partir AVEC le batch frenchy (sinon golden périmé sur checkout propre). `biped-golden.snap` = pur-moi (12/12). Browser playwright verrouillé par session // → vérif par test de rendu déterministe (suite 4623/0).

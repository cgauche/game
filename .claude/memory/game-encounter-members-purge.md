...
- **Authoring = `buildEncounter`/`buildEncounters` (`src/state/encounterAuthoring.ts`, PUR)** : terse `{ id, enemies:[…], hidden?, surprise?, onVictory? }` → `{ entities, encounter(members) }`. Id stable `enemy-<encId>-<i>`. SOURCE UNIQUE (pas de dupe) :
  ...
  - générateur d'arène : `scripts/arene/lib.mjs#scene()` importe `buildEncounter` et tourne via **tsx** (`tsx scripts/arene/generate.mjs`, plus `node`).
...
Recette navigateur passée (3250 tests + 0 erreur console) : zone1 6 ennemis visibles→combat, Futaie harde cachée+bande visible, forgeron `Nains`→`Nains`. Voir [[game-arene-editor-data-project]], [[game-arene-data-driven]].

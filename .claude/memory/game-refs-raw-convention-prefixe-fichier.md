...
**Cliquet de graphie** (`scripts/raw/graphy-stock.json`, stock NOMINATIF : une entrée par SITE, clé `famille + fichier + réf + occurrence`) : `chDot`, `bareFolio` et `bookNoChapterSrc` n'y ont aucune entrée — toute occurrence y échoue nominativement. Seule famille encore déclarée : `chapterBoundaryFolio` — **52 sites** (mesuré 2026-09-12, #1711 T2), sous `src/` et sous `docs/raw/`. Voir aussi [[game-atlas-raw-doc]], #586 (borne haute), #456 (_GoBack).

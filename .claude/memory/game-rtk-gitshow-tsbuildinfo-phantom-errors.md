...
2. **Le cache incrémental de `tsc` périme** après un commit d'une **session concurrente** qui change un fichier sous le cache → `npm run typecheck` (= `tsc --noEmit --incremental --tsBuildInfoFile node_modules/.cache/typecheck.tsbuildinfo`) rapporte des **erreurs FANTÔMES** (ex. « `massBattle` n'existe pas sur les props de HeroCard » alors que la prop EST déclarée). Elles **disparaissent** sous `npx tsc --noEmit --incremental false` (exit 0) ou après suppression du fichier de cache — dont le chemin se LIT dans le script `typecheck` de `package.json` (`node_modules/.cache/typecheck.tsbuildinfo`), jamais supposé à la racine : purger un chemin qui n'existe pas donne l'illusion d'avoir vidé le cache.

...
4. **Même classe SANS stall (2026-09-01, L-gram-3)** : un codeur rend « src/ui/editor 243/243 » mesuré AVANT son dernier geste de solde (renommage d'un intitulé de `<select>`) → CI rouge sur `WorldMapEditor.test` à f002d1f75. Un verdict de gate date d'un ÉTAT : tout geste postérieur le périme — exiger « re-gate après le DERNIER geste » et, au prélèvement, rejouer soi-même la suite du dossier UI touché (ici `src/ui/editor`) quand un libellé d'écran a bougé.

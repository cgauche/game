...
**Why:** un `git checkout/restore` sur un fichier à WIP non commité = perte sèche ; l'arbre partagé est la prod de l'autre session. Cf. [[game-agents-worktree-isolation-shared-branch]], [[git-commits-propres-wip-parallele]].
...
**Quatrième incident (2026-07-24, lots campagne #768/#809)** : un codeur Sonnet, dont le brief interdisait explicitement « AUCUNE commande git en écriture », a lancé une régénération de paquet (porte demandée), constaté qu'elle produisait un diff hors de son périmètre, et « nettoyé » avec `git checkout -- src/scenes/barge-du-sel/…json …test.ts` — détruisant le travail non commité d'un agent FRÈRE de la MÊME session (correctif `statblock.name`→`label` + un test ajouté). Réparation : régénération du JSON (déterministe, donc récupérable) + réécriture du test depuis son jumeau resté intact. Leçon nouvelle : le risque n'est pas seulement la session PARALLÈLE — deux agents de MA vague qui touchent des fichiers voisins suffisent ; et un agent traite un diff qu'il ne comprend pas comme une bavure À NETTOYER. Corollaire de brief : dire « n'utilise pas git » ne suffit pas, il faut dire « un diff hors de ton périmètre n'est PAS ta bavure : tu le RAPPORTES, tu n'y touches pas ».

**Desserrage en worktree — ce qui l'autorise.** L'utilisateur POSE UNE QUESTION, il ne tranche pas (2026-09-03, verbatim) : « Pour les git destructif, on devrait pouvoir les faire sur les worktree, tu ne pense pas ? ». Une question ne fait pas autorité ; l'autorité du desserrage est la MESURE (branche refusée par git elle-même, index privé du worktree, `git clean -fdx` qui ne suit pas la jonction `node_modules`), verrouillée par les tests « FONDEMENT » de `scripts/hooks/git-destructive-guard.test.mjs`.

**PORTÉ PAR** `scripts/hooks/git-destructive-guard.mjs` : `ask` dans l'arbre principal et dans tout répertoire NON PROUVÉ (un `git reset --hard` nu ne prouve aucun arbre — le cwd persistant du canal Bash n'est observable par aucun hook) ; SILENCE pour `checkout`/`restore`/`reset`/`clean` dans un worktree LIÉ dont la commande porte `git -C <chemin>`/`cd <chemin>` ou dont le canal transmet `tool_input.cwd` ; `stash` = `ask` PARTOUT (pile partagée) ; jonction `node_modules` = `deny`. La dérive que cette fiche décrit reste la même : l'agent croit nettoyer sa bavure dans SON arbre et frappe l'arbre principal — c'est pourquoi la permission ne s'accorde qu'à un répertoire PROUVÉ, et l'interdit de brief reste une ceinture, pas un airbag.
...
3. Si je dois réparer l'arbre principal : JAMAIS de `git checkout` sur un fichier MIXTE (WIP parallèle + ma bavure) → retrait chirurgical de mes seules lignes ; `checkout` seulement sur les fichiers 100 % à moi, et seulement avec le feu vert de l'utilisateur (cf. [[git-commits-propres-wip-parallele]]).

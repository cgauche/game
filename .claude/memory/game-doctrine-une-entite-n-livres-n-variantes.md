...
- Tickets : **#563** (capacité d'expression de `source`), **#564** (l'axe module — 12 talents/traits dont le jeu ignore la variante AA), **#560** (folios réellement faux), **#565** (un sort affiche la desc d'un autre), **#566** (index du LDB amputé du bloc D-I, et chapitre des Traits sans marqueurs → folios irréfutables par construction).
- Contraintes de forme établies : `page: number[]` est FAUX (un emplacement est une paire livre+folio) ; `refs[0]` doit ancrer la `desc` ; les emplacements ≥1 doivent être **auto-attestants** (label retrouvé dans le span, ou `quote` authoré prouvé verbatim) ; deux trous permissifs à réparer dans le même geste (`citationCoverage.mjs:16` et `folioIntegrity.mjs:268` excluent silencieusement la forme liste) ; `registry.ts:92-95` re-déclare `CodexSource` au lieu d'importer `SourceRef`.

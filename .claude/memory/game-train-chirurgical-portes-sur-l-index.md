...
**8e occurrence (2026-09-12, fermeture de #1709/#1708) — un `const` de MODULE dont le littéral nomme un fichier EST une entrée de stock nominatif :** le pre-commit a refusé le commit de fermeture (« STOCK NOMINATIF qui NAÎT ou GRANDIT : `toutes.test.mjs` +1 — ex. `argv: ['node', 'toutes.mjs']` ») sur un helper `const jouerAvecPrerequis = … principal({ argv: ['node', 'toutes.mjs'] })` de portée module, alors que quatre recopies IDENTIQUES du même littéral dans des corps de `test(…)` ne comptaient pas (`scripts/guards/lib/stocksNominatifs.mjs:20-21` « un PORTEUR est un littéral atteignable depuis une liaison de MODULE » ; `:38-43` un littéral dans un corps de fonction est une DONNÉE LOCALE ; `:105` `NOM_NU` fait de `'toutes.mjs'` un nom). Mon geste réflexe « hisser une constante de module partagée pour faire DÉCROÎTRE la liste » a été RÉFUTÉ par le codeur avec la règle en main et MESURÉ (+1 avant, +1 après : la constante déplace l'entrée, elle ne la retire pas). Sortie : le littéral devient une DONNÉE DE CAS (paramètre du helper, écrit dans le corps de chaque test) → net 0. Règle : avant de prescrire « mutualiser » pour un refus de stock, lire la définition du PORTEUR dans le détecteur ; et deux gates de gates (`toutes.mjs` + ses tests) se rejouent depuis un worktree PROPRE quand l'arbre principal porte un fichier étranger non committé (`?? Source/…jpg` de l'utilisateur → `[gates] REFUS — l'arbre porte 1 chemin non committé au périmètre de la clé`) : les justificatifs vivent dans `<git-common-dir>/wfrp-justificatifs/` keyés par CONTENU, un run vert en worktree justifie le push de la même tête.

...
metadata: 
  ...
  modified: 2026-08-23T11:47:21.832Z
---
...
**Piège qui ANNULE le stage par hunk (2026-08-23, commit `bb824bafb` d'une session voisine)** :
`git commit -- <chemin>` (pathspec au commit) prend le contenu du **WORKING TREE** du chemin, PAS
l'index — la ligne staged chirurgicalement n'a servi à rien et un hunk d'une autre session
(`"docs:structures"` de #1465) est parti sous son message. Règle : après `git apply --cached mon.patch`,
committer **SANS pathspec** (`git commit -m …`, l'index seul fait foi) ; le pathspec de commit n'est
sûr que pour un fichier 100 % à moi. Vérifier avant : `git diff --cached --stat` = exactement mes
fichiers/hunks, et `git diff --stat` (non-staged) contient encore TOUT le WIP voisin.

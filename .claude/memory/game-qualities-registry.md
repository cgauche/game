...
⚠️ **CORRECTION 2026-07-05 (vérifié au code)** : l'architecture a changé depuis — il n'y a **plus de dossier `src/engine/qualities/defs/`** ni de `QualityDef` portant les champs d'effet (`attackMod`, `armourReduction`, `damageDR`…) directement dans le registre. Les objets portent des `QualityInstance{id, value?}` STRUCTURÉES (plus de chaîne « id value » reparsée) ; toute la MÉCANIQUE vit dans **`src/data/qualities.json`** (`passive: GameOp[]` — `weaponRollMod`/`weaponDamageMod`/`armourPierce`/`critOnRoll`/`testMod` — + `capabilities` + `effects`), lue **PAR ID** via `qualityById` (`src/data`). `src/engine/qualities/registry.ts::QUALITIES` est désormais DÉRIVÉ 1:1 de la donnée et ne porte plus que le **libellé d'affichage** (`{key: label}`) — le keying par label FR canonique ne sert plus qu'à l'AFFICHAGE, jamais à la résolution mécanique (qui passe uniquement par l'id).

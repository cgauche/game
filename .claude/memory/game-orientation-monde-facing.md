...
- `BodyToken` (src/gameIso/BodyToken.tsx) = **coquille de positionnement unique** (ombre+ancrage+échelle+mort) ; `token()`/`tokenNode()` (IsoStage) + `EntityToken` délèguent.
...
- Legacy supprimé au passage : chemin de rendu monolithique en combat (mort — `bodyPlanOf` ne renvoie jamais `'monolithic'`), `hasCreatureViews`. `creatureView`/`creatureViews.json` GARDÉS (outillage QC). Cf. [[game-supprimer-legacy]].

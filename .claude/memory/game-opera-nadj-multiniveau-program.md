...
Chantier : implémenter le scénario **« Une nuit à l'Opéra »** (`Source/Warhammer v4 - Nuits agitees & dures journées/08 - Une nuit à l'Opéra.md`). Plan approuvé : `~/.claude/plans/tout-m-interesse-harmonic-token.md` (9 lots V/0/1/3/2/L/4/S/C).
...
**Moteur multi-niveaux (Lot V) LIVRÉ** : `Scene.tiles → levels: Level[]` (z=0=sol, `stairs` bidirectionnels) ; `tileAt`/`isWalkable(scene,x,y,z)` z-aware ; projection `iso.ts` (`tileCenter`/`depth`/`screenToTileAtZ`/`LEVEL_H`) ; `path.ts` uniformément 3D (voisins 4-adj même-z + transitions stairs ; convention z=0 = clé sans suffixe → non-régression byte-identique au sol) ; `partyPos` porte z, `moveParty` z-aware ; entités `SceneEntity.z` ; éditeur : sélecteur d'étage + outil escalier (Palette 🪜) + `validateScene` avertit étage/case incohérents. **Multi-niveaux entièrement éditable.**
...
- **Un escalier est STRUCTUREL, pas un prop décoratif** — `gameIso/stairs.ts::stairSeg` rend une vraie volée (giron/contremarche, démarre une case en avant, grimpe en diagonale) intégrée au tri IsoStage.

...
- ✅ **Lot C1** (7bf5be31) : Puissance d'armée = Blessures d'un Combattant inanimé (`inanimateCombatant`, `wounds.current`=Puissance, `wounds.max`=départ). Deltas de Scène = GameOp `heal`/`wounds` ; plafond « pas au-dessus du départ » (l.135) = plafond naturel de `heal→max`. Clash 10+DR(min5). Supprimé : `might`/`startMight` numériques, `applyMightDelta`.
- ✅ **Lot C2a** (9192d15d) : activités de bataille → `ActivityDef` (contexte `bataille`/`bataille-round`) ; capacités génériques `combined`/`assisted`/`requires`/`grantsFlag` ; canal de jet UNIQUE `PendingActivity`+`RollShell`. Supprimé : `BattleActivityDef`, `BattleScene`, `PendingBattleTest`, **`BattleTestModal.tsx`**, tout le vocabulaire `ActivityOutcome`/`sceneDeltas`/`battleActivityById`.
- ✅ **Lot C2b = ex-Lot B** (22390e5b) : budget `max 3` PARTAGÉ par construction (`interlude.perHero.left`, source unique `consumeActivity`). Prépa (contexte `bataille`) décrémente `left` ; Scènes de Round non. Supprimé : `prepCount`, phase `'inspire'`(→`'prep'`), 2e budget. Décision user : **prépa = activités d'interlude** (bataille sans interlude → Round 1 sans prépa).

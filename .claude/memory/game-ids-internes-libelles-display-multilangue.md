...
- **Y COMPRIS à l'AUTHORING** : l'auteur écrit des IDS (`hache-d-armes`, jamais « Hache » ; « Marchande » = interdit) ; les résolveurs de `scripts/campagne/lib.mjs` (outillage d'authoring PARTAGÉ par toutes les campagnes, Arène comprise) sont des VALIDATEURS id-only fail-fast — un id valide passe tel quel, tout le reste `throw` en pointant où trouver l'id (Compendium/catalogue) ; les pickers éditeur/codex affichent le libellé et stockent l'id (seule aide à la saisie légitime).
...
Prolonge [[game-label-id-migration-complete]] ; relié à [[credo-exemples-calibrants]].

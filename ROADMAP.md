...
- ✅ **T5 — Sous-système Psychologie COMPLET (P1-P4) LIVRÉ** (2026-06-07, 1181 tests verts) : `engine/psychology.ts` (pur) + `engine/groups.ts`. Spec : `…/specs/2026-06-07-psychologie-design.md` ; plans P1/P2/P3 sous `…/plans/2026-06-07-psychologie-*.md`.
  ...
  - **P2 — Frénésie** (`21` l.31-36) : entrée par **Test de FM** (héros = modale `pendingFrenzy` + bouton « 🐗 Frénésie » ; IA = auto) ; **+1 BF**, **immunité psy**, **attaque CC gratuite/Round** (`aiAvailableFreeAttack`), **cible imposée la plus proche** (IA), **fin → Exténué**.
...
- ✅ **Combat monté** (`14` l.212-225) — *COMPLET (les 7 ajouts du RAW), testé.* `state/mount.ts` (module-feuille pur) :
  ...
  - ✅ **Clips d'ATTAQUE montés** *(2026-06-11 — clôt le différé)* : gestes EN SELLE dédiés (`mountedAttackClip`/`mountedParryClip` + `seatedClip`, `useRigAnim {seated}`) — charge lance couchée (la lance RESTE en arrêt, le poing projette), taille/estoc/coup 2 mains depuis le port dressé, mise en joue arbalète/arme à feu ; **aucun geste assis ne touche bassin/jambes** (ancrage selle) et le cavalier **ne joue plus le clip de marche** (la monture marche pour deux) ; calibrage à la sonde FK (angle monde additif ; profil natif : epauleD NÉGATIF = poing en avant), QC planches + section « En selle » de `clip-anim-gallery`. Reste cosmétique pré-existant : main qui flotte légèrement en profil (avant-bras non dessiné = trait du rig, pas spécifique à la monture).
  ...
  *(Queue/Langue : ✅ attaques de créature, Jalon 1. Immunité Psychologie : ✅ T5. Mécanique, rendu en selle ET clips montés = complets, testés et recettés. **Plus aucun reste sur ce poste.**)*
...
- ✅ **Consommables en combat** : action **« 🧪 Utiliser »** dans la hotbar (`battleUseItem` + `engine/consumables.ts`) — effet **parsé du `desc`** du trapping (LDB p.307) : Potion de guérison = soin du **Bonus d'Endurance**, Potion de vitalité = retrait de l'État **Exténué** ; objet consommé, coûte l'Action ; liste groupée (plusieurs potions → ×N).

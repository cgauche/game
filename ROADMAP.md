...
- ✅ **Combat monté** (`14` l.212-225) — *COMPLET (les 7 ajouts du RAW), testé.* `state/mount.ts` (module-feuille pur) :
  ...
  - 🚧 **Rendu en selle** : le cavalier est rendu SUR sa monture (même tuile, au-dessus). **⚠ PARKÉ — moche** : plaquer un rig humanoïde DE FACE sur le gabarit cheval donne « un petit soldat de face planté sur le dos », pas un cavalier. À refaire dans une vraie passe d'art (facing en VRAI profil aligné sur la monture + échelle + pose assise calée à la main + layering de profondeur jambe-lointaine-derrière). Outil de QC hors-navigateur prêt : `scripts/_qc-monture.mts` (rastérise la compo IsoStage en PNG).

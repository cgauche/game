...
- 🚧 **Combat monté** (`14` l.212-225) — *cœur LIVRÉ + testé ; reste rendu/éditeur/IA.* `state/mount.ts` (module-feuille pur) :
  ...
  - ✅ **Rendu en selle** : le cavalier n'est plus dessiné au sol — il est rendu SUR sa monture (même tuile, échelle cavalier, remonté à la selle, sans ombre/anneau propres, au-dessus de la monture). *(offset de selle à affiner à l'œil.)*
  ...
  - ⏳ **Reste** : (c) **IA** : un PNJ DÉCIDE d'enfourcher une monture libre / la monture sans Trait *Nerveux* agit seule (l.221) ; (d) **ciblage** cavalier↔monture (modale de choix au clic sur la case partagée, l.219) ; (e) **charge montée — dégâts** F + Taille de la monture (l.223). *(c+e vivent dans `combatFlow.ts`.)*

...
export const DES_HORS_PORTE_STOCK = new Map([
  ...
  ['src/data/schemas/defs-scenes/worldmap.ts', { n: 4, kind: 'dette', why: 'mesuré : rng.int×4. F (désignation `rng.int` en donnée authorée) -> #1508 T6.' }],
  ['src/data/schemas/defs/props.ts', { n: 1, kind: 'dette', why: 'mesuré : rng.int×1. F (désignation `rng.int` en donnée authorée) -> #1508 T6.' }],
  ['src/data/schemas/defs/surincantation.ts', { n: 6, kind: 'dette', why: 'mesuré : rng.int×6. F (désignation `rng.int` en donnée authorée) -> #1508 T6.' }],
  ['src/data/schemas/defs/vehicles.ts', { n: 1, kind: 'dette', why: 'mesuré : rng.int×1. F (désignation `rng.int` en donnée authorée) -> #1508 T6.' }],
  ['src/data/schemas/grammaire/avancement.ts', { n: 1, kind: 'dette', why: 'mesuré : rng.int×1. F (désignation `rng.int` en donnée authorée) -> #1508 T6.' }],
  ['src/data/schemas/grammaire/ref.ts', { n: 1, kind: 'dette', why: 'mesuré : rng.int×1. F (désignation `rng.int` en donnée authorée) -> #1508 T6.' }],
  ...
  ['src/state/combatEffects.ts', { n: 23, kind: 'dette', why: 'mesuré : applyOps×6, resolveFormula×6, rng.int×2, applyFaimTest×1, applyFall×1, applyManannFactor×1, applySoifTest×1, d10×1, d100×1, drawWaterDisease×1, roll×1, traumaOnImpossibleAmbition×1. B (magnitudes d\'`applyOps`/`resolveFormula`), C (magnitudes d\'applier) -> #1508 T2/T5.' }],
  ['src/state/combatFlow.ts', { n: 60, kind: 'dette', why: 'mesuré : resolveFormula×13, applyOps×9, rng.int×5, resolveCritique×3, rollGrappleForce×3, rollTest×3, d10×2, resolveRanged×2, rollDisengageAttack×2, rollMeleeAttacker×2, rollOups×2, rollStructureCritical×2, applyFall×1, applyHullCritical×1, opposedTest×1, resolveCounterspell×1, resolveRun×1, resolveTrample×1, rollArtillerySalveMisfire×1, rollMeleeDefender×1, rollMiscast×1, rollRangedAttacker×1, rollWindsOfMagic×1, scatter×1. B (magnitudes d\'`applyOps`/`resolveFormula`), D (sauvegardes du héros), E (dispersion), + les Tests déjà comptés au registre -> #1508 T2/T3/T6.' }],
  ...
  ['src/state/travelFlow.ts', { n: 12, kind: 'dette', why: 'mesuré : d10×2, d100×2, applyFall×1, applyOps×1, declareDisease×1, forcedMarchTest×1, forcedPaceBeastCheck×1, forcedPaceCheck×1, resolveMountedDay×1, rollTest×1. C (magnitudes de route), chute (`applyFall`) -> #1508 T2/T5.' }],

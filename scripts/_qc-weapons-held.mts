...
const cells: Cell[] = [
  ...WEAPON_FORMS.map((f) => ({ slug: f.slug, label: f.label, svg: rig({ weapons: [{ name: f.label, type: f.type, damage: { plusBF: false, flat: 4 }, qualities: [], shape: f.slug } as Weapon], armour: [] }) })),
  ...SHIELD_FORMS.map((s) => {
    const sh = { name: s.label, type: 'melee', damage: { plusBF: false, flat: 0 }, qualities: [{ id: 'protectrice', value: 1 }], shape: s.slug } as Weapon;

...
WEAPONS.forEach((name, r) => {
  ...
  VIEWS.forEach((vw, c) => {
    const w = wpn(name);
    const inner = renderToStaticMarkup(React.createElement(RigSprite, { appearance: { species: 'Humain' as RigSpeciesId, sex: 'M', build: 0.5, seed: 4 }, equip: { weapons: [w], armour: [] }, career: 'Soldat', view: vw.v, pose: weaponRest(w) }));

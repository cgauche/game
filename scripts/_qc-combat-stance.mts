/** Rend la POSE DE PORT (carryPose, = pose de combat au repos) par arme. → public/qc/stance.png
 *  Contrairement aux autres planches, applique carryPose (ce que le RigToken montre en combat). */
...
const wpn = (name: string): Weapon => ({ name, type: name === 'Arc' || name === 'Arbalète' ? 'ranged' : 'melee', damage: '+0', qualities: [] });
...
WEAPONS.forEach((name, r) => {
  ...
  VIEWS.forEach((vw, c) => {
    const w = wpn(name);
    const inner = renderToStaticMarkup(React.createElement(RigSprite, { appearance: { species: 'Humain', sex: 'M', build: 0.5, seed: 4 }, equip: { weapons: [w], armour: [] }, career: 'Soldat', view: vw.v, pose: carryPose(w) }));
...
const full = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}"><defs>${DEFS}</defs><rect width="${W}" height="${H}" fill="#11141c"/><text x="12" y="18" font-size="14" fill="#d8a93b" font-family="sans-serif">Pose de combat (carryPose) par arme</text>${cells.join('')}</svg>`;

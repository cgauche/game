...
async function main() {
  ...
  try {
    ...
    for (const w of args.widths) {
      ...
      console.log(`combat ${w}px — ${m.vcBtns.length} commande(s) de vue, piste ${m.piste.clientWidth}/${m.piste.scrollWidth}px dans une bande de ${m.piste.bande}px, chevauchement fil×frise ${m.feedXfrise ? m.feedXfrise.ox + '×' + m.feedXfrise.oy + 'px' : 'aucun'} → ${d.length ? d.length + ' défaut(s)' : 'OK'}`);
      echecs.push(...d);
    }
  } finally {
    await session.close();

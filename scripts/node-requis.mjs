...
// ne se déclare pas « principal » sous un Node plus ancien et sort 0 sans rien faire. Les PORTES
// refusent donc avant tout verdict :
//   - les hooks shell de `scripts/git-hooks/`, avant leur `.mjs` ;
//   - `npm run gates` : premier import de `scripts/gates/toutes.mjs` ;
//   - `npm install`/`npm ci` : `.npmrc` `engine-strict`.
// Un script lancé seul (`npm run docs:check`) n'est pas une porte : il n'est pas couvert. `.npmrc`
// `node-options` le couvrirait, mais npm y REMPLACE le `NODE_OPTIONS` de l'appelant.

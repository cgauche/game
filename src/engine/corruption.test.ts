...
import {
  ...
  attachMutation, mutationCharDelta, mutationMovementDelta, mutationArmourBonus, mutationTestMod,
} from './corruption';
...
describe('seuil & limites (l.80/95)', () => {
  ...
  it('limites : physiques > BE ou mentales > BFM → damné', () => {
    const phys = (n: number) => Array.from({ length: n }, (_, i) => ({ label: `m${i}`, kind: 'physique' as const, roll: 1 }));
    ...
    const ment = (n: number) => Array.from({ length: n }, (_, i) => ({ label: `m${i}`, kind: 'mentale' as const, roll: 1 }));
...
describe('mutationKindFor — d100 corps/esprit par espèce (l.87-91)', () => {
  it('Humain : corps 01-50, esprit 51-100', () => {
    expect(mutationKindFor('Humain', 50)).toBe('physique');
    expect(mutationKindFor('Humain', 51)).toBe('mentale');
  });
  it('Nain 01-05, Halfling 01-10, Elfe jamais physique', () => {
    expect(mutationKindFor('Nain', 5)).toBe('physique');
    expect(mutationKindFor('Nain', 6)).toBe('mentale');
    expect(mutationKindFor('Halfling', 10)).toBe('physique');
    expect(mutationKindFor('Haut Elfe', 1)).toBe('mentale');
    expect(mutationKindFor('Elfe sylvain', 100)).toBe('mentale');
  });
  it('espèce inconnue → traité comme Humain', () => {
...
describe('effets de mutation lus à la volée', () => {
  ...
  it('PA naturels apAll + apLocations', () => {
    const c = hero();
    attachMutation(c, { label: 'Écailles épineuses', kind: 'physique', roll: 78, apAll: 1 });
    attachMutation(c, { label: 'Cornes asymétriques', kind: 'physique', roll: 83, apLocations: { tete: 1 } });
  ...
  it('mods de Tests : compétence nommée + Tests d\'une caractéristique (testValue)', () => {
    ...
    expect(mutationTestMod(c, 'Pistage', 'I')).toBe(10);
    expect(testValue(c, 'Charme')).toBe(30 - 20); // Soc 30, Tests de Sociabilité −20
  });

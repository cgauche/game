 ...
 * dupliquée ailleurs.
 */
import type { Combatant, HitLocation, ArmourBypass } from './types';
...
/** Somme des PA des pièces portées à `loc` dont le nom matche `material`. */
function materialAPAt(c: Combatant, loc: HitLocation, material: RegExp): number {
  return (c.items ?? [])
    .filter((i) => i.equipped && i.kind === 'armor' && i.locs?.includes(loc) && material.test(`${i.name} ${i.subType ?? ''}`))
    .reduce((s, i) => s + piecePA(i), 0);
}

/** PA des armures MÉTALLIQUES portées à `loc` (matière par nom). */
export const metalAPAt = (c: Combatant, loc: HitLocation): number => materialAPAt(c, loc, /maille|plate|m[ée]tal|metal/i);

/** PA des armures de CUIR portées à `loc`. */
export const leatherAPAt = (c: Combatant, loc: HitLocation): number => materialAPAt(c, loc, /cuir|fourrure/i);

...
export interface ResolvedQuality {
  ...
  /** Indice porté par la référence (`QualityInstance.value`) — paramètre les effets de la qualité
   *  (`state/triggeredEffects::withArg`, template `$indice`) et son affichage (`qualityRefLabel`). */
  indice?: number;
}

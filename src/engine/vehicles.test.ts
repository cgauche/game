...
describe('dédup trappings ⊥ vehicles + résolution unifiée', () => {
  ...
  it('hull (E/Blessures) posé là où le RAW EDOC ch.4 correspond sans ambiguïté', () => {
    expect(vehicles.find((v) => v.id === 'diligence')!.hull!.char).toEqual({ E: 45, B: 50 });
    expect(vehicles.find((v) => v.id === 'charrette')!.hull!.char).toEqual({ E: 25, B: 10 });

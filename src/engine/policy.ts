...
export const OPTIONAL_RULES: OptionalRule[] = [
  ...
  {
    id: 'combat-voice-range-m',
    label: 'Portée de voix (commandement)',
    ref: 'AA 13 l.35 / LDB 09 l.128 — « à portée de voix » jamais chiffrée, valeur maison',
    group: 'Combat',
    kind: 'param',
    default: 50,
    min: 2,
    max: 200,
    step: 2,
    hint: 'Distance en MÈTRES à laquelle un ordre crié porte : « aider une équipe qui utilise une arme possédant le Défaut Arme d’équipe à portée de voix » (AA 13 l.35) — le canon ne chiffre jamais cette portée. Défaut 50 m (≈ 25 cases à 2 m/case).',
  },
  {
  ...
  {
    id: 'market-cite-stock',
    label: 'Cité : quantité d’un objet en stock',
    ref: 'LDB 59 l.34 — quantité en cité non chiffrée, valeur maison',
    group: 'Marché',
    kind: 'param',
    default: 99,
    min: 1,
    max: 999,
    step: 1,
    hint: 'Quantité disponible d’un objet en stock dans une CITÉ : « les cités en possèdent autant que le MJ le juge approprié » (LDB 59 l.34) — le canon ne chiffre rien (village 1, ville 1d10). Défaut 99 = stock pratiquement illimité ; baisser pour une cité rationnée.',
  },
  {
    id: 'tavern-games',
  ...
  },
  {
    ...
    kind: 'flag',
    default: false,
    hint: 'Les Vents de Magie révisent les règles d’incantation du Livre de base : « Elles remplacent celles du Livre de Règles de WFJDR » (VDM 02 l.5), « Bien entendu, vous êtes libre d’utiliser celles que vous souhaitez » (l.7). Activé : les Dégâts d’un Projectile magique valent Dégâts du Sort + Bonus de Force Mentale du lanceur, sans le DR du Test d’Incantation (l.68) ; « Puissance totale », sur une Incantation Critique, ajoute le chiffre des dizaines du lancer d’Incantation au DR pour obtenir une Surincantation (l.55) ; les Incantations Imparfaites se tirent sur les tableaux des Vents de Magie (l.218-263) — dont la rangée « Marqué par la Magie », qui tire sur les Marques arcaniques du Vent du lanceur, ou relance sur le tableau Majeur si sa tradition n’en a pas ; la Surincantation suit le Tableau de Surincantation par palier de DR (l.194-215), les DR au-delà du NI étant dépensables un par un ; et près d’une Influence corruptrice, tout lancer d’Incantation ou de Focalisation raté impose une Imparfaite Mineure — Majeure si une Mineure était déjà due (l.157-159). Désactivé par défaut (règles du Livre de base).',
  },
  {

...
describe('spellDuration — round-trip parse∘format = identité', () => {
  const ds: SpellDuration[] = [
    { kind: 'instant' },
    { kind: 'rounds', value: 6 },
    { kind: 'rounds', value: { bonusOf: 'FM' } },
    { kind: 'clock', value: 1, unit: 'hours' },
    { kind: 'clock', value: { charOf: 'FM' }, unit: 'minutes' },
    { kind: 'clock', value: { bonusOf: 'Soc' }, unit: 'days' },
    { kind: 'untilDawn' },
...
describe('spellDuration — parse prose réelle (sanity)', () => {
  it('échelles', () => {
    ...
    expect(parseSpellDuration('(Bonus de Force Mentale) Rounds')).toEqual({ kind: 'rounds', value: { bonusOf: 'FM' } });
    ...
    expect(parseSpellDuration('(Force Mentale) minutes')).toEqual({ kind: 'clock', value: { charOf: 'FM' }, unit: 'minutes' });

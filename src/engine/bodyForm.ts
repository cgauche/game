/**
 * Forme du corps d'une ESPÈCE → Tableau de Localisation (LDB 76 p.312). SOURCE DE VÉRITÉ RÈGLES,
 ...
 *
 * Seules les espèces NON humanoïdes sont listées (défaut = humanoïde, table par défaut LDB 76 p.312 —
...
export const SPECIES_BODY_SHAPE: Record<string, BodyShape> = {
  ...
  stegadon: 'quadrupede',
  // Ailés / oiseaux (LDB 76 p.312 : ailes = bras) — même mécanique que le quadrupède.
...
/** Forme du corps d'une espèce (LDB 76 p.312) — défaut humanoïde. `undefined`/inconnue ⇒ humanoïde. */

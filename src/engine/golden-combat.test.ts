...
describe('Golden master — combat (iso-comportement du registre de qualités)', () => {
    ...
    for (const q of QSETS) {
      for (const defense of ['parade', 'esquive', 'none'] as const) {
        for (let seed = 1; seed <= 25; seed++) {
          const atk = mk('A', { CC: 55, F: 40 }, { name: 'Épée', type: 'melee', damage: { plusBF: true, flat: 4 }, qualities: q });
          const def = mk('D', { CC: 45, E: 35 }, { name: 'Épée', type: 'melee', damage: { plusBF: true, flat: 4 }, qualities: ['Défensive'] }, 2);
  ...
  it('distance : portée × qualités × seeds — snapshot stable', () => {
    ...
    for (const q of [[], ['Perforante'], ['Pointue'], ['Empaleuse']]) {
      for (let seed = 1; seed <= 25; seed++) {
        const atk = mk('A', { CT: 55 }, { name: 'Arc', type: 'ranged', damage: { plusBF: false, flat: 9 }, range: 50, qualities: q });

 ...
 * (hors `*.test.ts`) importe quoi que ce soit de `src/state` — `from '../state/…'`, `import('../state/…')`,
 * y compris depuis un sous-dossier (`'../../state/…'`).
 *
 ...
 *
 * Les fichiers de TEST sont exclus du scan : ils exercent légitimement le runtime `state`
 * (`runSpellFlowLines`, `applyTriggeredEffects`…) — ce sont des consommateurs, pas le moteur.
 */
...
const ALLOWLIST = new Set(['corruption.ts']);

/** Tout import (statique `from`, dynamique/inline `import(`, ou side-effect `import '…'`) ciblant un
 *  chemin `…/state/…` (un ou plusieurs `../`). */
const STATE_IMPORT = /(?:\bfrom\s+|\bimport\s*\(\s*|\bimport\s+)['"](?:\.\.\/)+state\//;
...
describe('pureté du moteur — src/engine ne dépend JAMAIS de src/state (#8, règle 3)', () => {
  const files = engineSources();

  it('aucun fichier engine (hors tests) n\'importe de src/state (sauf allowlist)', () => {
    const offenders: string[] = [];
    for (const f of files) {
      const base = f.split('/').pop()!;
      if (ALLOWLIST.has(base)) continue;
      const code = stripComments(readFileSync(`${ENGINE_DIR}/${f}`, 'utf8'));
      if (code.split('\n').some((l) => STATE_IMPORT.test(l))) offenders.push(f);
    }
    expect(
      offenders,
      `Inversion de couche engine→state interdite (#8). Fichiers fautifs :\n  ${offenders.join('\n  ')}\n` +
        'Le moteur doit importer engine/flowCore (pas state/flow), ou le type partagé doit migrer en couche neutre.',
    ).toEqual([]);
  });

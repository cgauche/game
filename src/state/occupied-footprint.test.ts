...
const flatScene = (w: number, h: number): Scene =>
  ({ id: 's', label: 's', dimensions: { w, h }, ambiance: 'exterieur', layers: [{ z: 0, tiles: new Array(w * h).fill('herbe') }], entities: [], dialogues: [], triggers: [], encounters: [], flags: {} }) as unknown as Scene;

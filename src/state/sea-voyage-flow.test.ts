...
describe('registre cascadeAppliers — les 10 Tests d’équipage de VOYAGE (#275 Ronde 2 cran 3 — PILOTE RÉEL)', () => {
  ...
  function step(kind: string, sl: number, success = sl >= 1): CascadeStep {
    return { id: kind, kind, label: kind, result: { roll: 0, target: 0, sl, success }};
  }
...
describe('Mal de mer — #460 (MDG 14 l.211-222, câblage jamais branché, cycle de maladie réutilisé)', () => {
  ...
  it('applier `sea-mal-de-mer` : échec → contracté (malaise/nausée/persistant)', () => {
    const hero = get().party[0];
    hero.diseases = [];
    cascadeAppliers['sea-mal-de-mer'].apply(get, set, { id: 'x', kind: 'sea-mal-de-mer', label: 'x', result: { roll: 99, target: 40, sl: -6, success: false }}, hero, { steps: [], index: 0 });
  ...
  it('applier `sea-mal-de-mer` : réussite → rien', () => {
    const hero = get().party[0];
    hero.diseases = [];
    cascadeAppliers['sea-mal-de-mer'].apply(get, set, { id: 'x', kind: 'sea-mal-de-mer', label: 'x', result: { roll: 1, target: 40, sl: 6, success: true }}, hero, { steps: [], index: 0 });
...
describe('Tonneau d\'eau contaminé — #460 (MDG 14 l.209, `vessel.waterLitres` seul — la petite bière y échappe)', () => {
  ...
  it('applier `sea-tonneau-contamine` : échec → `sea.waterContaminated` posé (visible dès demain)', () => {
    ...
    set({ travelPlan: plan });
    const hero = get().party[0];
    cascadeAppliers['sea-tonneau-contamine'].apply(get, set, { id: 'x', kind: 'sea-tonneau-contamine', label: 'x', result: { roll: 99, target: 40, sl: -6, success: false }, meta: { diseaseId: 'flux-sanglant' } }, hero, { steps: [], index: 0 });
  ...
  it('applier `sea-tonneau-contamine` : réussite → rien', () => {
    ...
    set({ travelPlan: plan });
    const hero = get().party[0];
    cascadeAppliers['sea-tonneau-contamine'].apply(get, set, { id: 'x', kind: 'sea-tonneau-contamine', label: 'x', result: { roll: 1, target: 40, sl: 6, success: true }, meta: { diseaseId: 'flux-sanglant' } }, hero, { steps: [], index: 0 });
  ...
  it('applier `sea-tonneau-expose` : boire au tonneau contaminé, échec → contracté', () => {
    const hero = get().party[0];
    hero.diseases = [];
    cascadeAppliers['sea-tonneau-expose'].apply(get, set, { id: 'x', kind: 'sea-tonneau-expose', label: 'x', result: { roll: 99, target: 20, sl: -8, success: false }, meta: { diseaseId: 'peste-noire' } }, hero, { steps: [], index: 0 });
  ...
  it('applier `sea-tonneau-expose` : réussite → rien', () => {
    const hero = get().party[0];
    hero.diseases = [];
    cascadeAppliers['sea-tonneau-expose'].apply(get, set, { id: 'x', kind: 'sea-tonneau-expose', label: 'x', result: { roll: 1, target: 20, sl: 8, success: true }, meta: { diseaseId: 'peste-noire' } }, hero, { steps: [], index: 0 });
    expect(get().party.find((h) => h.id === hero.id)!.diseases ?? []).toHaveLength(0);
  });
...
describe('Survitesse — « Ça va lâcher, capitaine ! » (#443, MDG 13 l.121-142)', () => {
  ...
  function step(kind: string, sl: number, success = sl >= 1, meta?: Record<string, number>): CascadeStep {
    return { id: kind, kind, label: kind, result: { roll: 0, target: 0, sl, success }, meta };
  }

...
describe('Étape « quantité » — la 6ᵉ interaction de la coquille', () => {
  it('une étape à `quantity` est l’interaction « quantite », et elle naît PRÊTE sur sa valeur d’ouverture', () => {
    const h = hero();
    const st = quantityStep({ id: 'q', kind: 'mesure-quantite', label: 'Combien ?', actorId: h.id, min: 1, max: 100, value: 42 })!;
    ...
    expect((quantityStep({ id: 'q2', kind: 'mesure-quantite', label: 'C', actorId: h.id, min: 5, max: 9 })! as CascadeStep).amount).toBe(5);
  });
  ...
  it('la porte REFUSE une plage vide (aucun nombre à poser) — jamais une fenêtre en impasse', () => {
    ...
    expect(() => quantityStep({ id: 'q', kind: 'mesure-quantite', label: 'C', actorId: h.id, min: 10, max: 3 })).toThrow();
  });
  ...
  it('le poseur d’état écrit le nombre BORNÉ sur l’étape courante, et l’applier le reçoit', () => {
    const h = hero();
    const st = quantityStep({ id: 'q', kind: 'mesure-quantite', label: 'Combien ?', actorId: h.id, min: 1, max: 100, value: 1 })!;
  ...
  it('résolution IMMÉDIATE : l’étape passe sur sa valeur d’ouverture, sans fenêtre ni défaut parallèle', () => {
    const h = hero();
    const st = quantityStep({ id: 'q', kind: 'mesure-quantite', label: 'Combien ?', actorId: h.id, min: 1, max: 100, value: 30 })!;
...
describe('Seconde lecture d’un jet — Test COMBINÉ (LDB 12 l.202-208)', () => {
  ...
  it('la PORTE transmet la déclaration jusqu’à l’étape : la fenêtre annonce ses DEUX cibles', () => {
    const h = hero();
    const st = monoStep({
      id: 'm', kind: 'mesure-quantite', label: 'Test combiné', actor: h,

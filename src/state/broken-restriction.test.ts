...
function setup(broken: boolean, enemyPos = { x: 9, y: 5 }) {
  const hero = { id: 'h', kind: 'hero', name: 'H', pos: { x: 5, y: 5 }, movement: 4, weapons: [{ name: 'Épée', type: 'melee', damage: '+4', qualities: [] }], conditions: broken ? [{ name: 'brise', value: 1 }] : [], characteristics: { FM: 40 }, skills: [], wounds: { current: 10, max: 10 }, advantage: 0, engagedWith: [], psychState: [] } as unknown as Combatant;
...
describe('Brisé — restriction d\'action (LDB 16 l.55)', () => {
  ...
  it('IA : un ennemi Brisé non Engagé FUIT (move s\'éloignant des héros)', () => {
    const enemy = { id: 'e', kind: 'enemy', name: 'E', pos: { x: 5, y: 5 }, movement: 4, weapons: [{ name: 'Épée', type: 'melee', damage: '+4', qualities: [] }], conditions: [{ name: 'brise', value: 1 }], wounds: { current: 10, max: 10 }, engagedWith: [] } as unknown as Combatant;

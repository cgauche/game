...
describe('openCodex — drill-in modale vs parcours plein écran', () => {
  ...
  it('réf cliquée en jeu (focus + écran de jeu) → modale, sans changer d’écran', () => {
    useGame.getState().openCodex({ category: 'talents', label: 'Sixième sens' });
    expect(useGame.getState().screen).toBe('campaign');
    expect(useGame.getState().codexOverlay).toEqual({ category: 'talents', label: 'Sixième sens' });
  });
  ...
  it('déjà sur l’écran Codex → déplacement en place (compendiumFocus), pas de modale', () => {
    ...
    useGame.getState().openCodex({ category: 'skills', label: 'Savoir' });
    expect(useGame.getState().screen).toBe('compendium');
    expect(useGame.getState().compendiumFocus).toEqual({ category: 'skills', label: 'Savoir' });
    expect(useGame.getState().codexOverlay).toBeNull();
  });

  it('cross-réf DANS la modale → plonge dans la modale (codexOverlay mis à jour)', () => {
    useGame.setState({ screen: 'campaign', codexOverlay: { category: 'talents', label: 'A' } });
    useGame.getState().openCodex({ category: 'spells', label: 'B' });
    expect(useGame.getState().screen).toBe('campaign');
    expect(useGame.getState().codexOverlay).toEqual({ category: 'spells', label: 'B' });
  });

  it('closeCodexOverlay ferme la modale', () => {
    useGame.setState({ codexOverlay: { category: 'talents', label: 'A' } });

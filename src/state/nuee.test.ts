...
describe('Nuée — Trait Essaim (LDB 85 l.199-200)', () => {
  it('build au spawn : ×5 PB, +10 CC, immunité Psychologie, flag swarm', () => {
    const c = statblockToCombatant({ name: 'Nuée de rats', char: { CC: 35, F: 30, E: 30, B: 5 }, traits: ['Nuée', 'Taille (Petite)'] }, 'x', { x: 0, y: 0 });
  ...
  it('sans le trait : ni swarm ni ×5', () => {
    const c = statblockToCombatant({ name: 'Rat', char: { CC: 35, B: 5 }, traits: ['Taille (Petite)'] }, 'x', { x: 0, y: 0 });
  ...
  it('+40 au tir CONTRE une nuée, et la Taille de la cible est ignorée', () => {
    const swarm = statblockToCombatant({ name: 'Nuée', char: { B: 5 }, traits: ['Nuée', 'Taille (Petite)'] }, 's', { x: 0, y: 0 });

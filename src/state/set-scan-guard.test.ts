...
// +1 (ASSISE #1443 : `partyFlow.libererPlace` REND sa place au héros qui sort du groupe
// (retrait, changement de meneur) — un `set({ scene })` littéral, et conditionnel : la scène n'est
// réécrite que si `releaseSeat` l'a vraiment changée (sinon même référence, aucun delta d'instance).
// Le geste d'assise du store, lui, n'en ajoute AUCUN : sa transaction passe par le callback
// `set((s) => …)` — c'est ce qui la rend atomique — et sa libération au déplacement se FOND dans le
// `set` de `partyPos` qui existait déjà. Aucun reset ad hoc de plus : `scene` n'est pas un pending.
// Mesure exacte 744/302).

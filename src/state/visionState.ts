...
import { Pt } from './path';
import { computeVisible, computeLightField, ambientScalar, baseSightTiles, darkSightTiles } from './vision';
...
export function computeStateVisible(s: VisionInput): Set<string> {
  ...
  // V2a : sources de lumière = ambiant seul (les torches portées/posées arrivent au lot lumière V2b).
  const light = computeLightField(scene, ambient, [], smoke);

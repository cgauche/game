...
export function advanceTurn(get: () => GameState, set: any) {
  ...
  set({ battle: { ...battle, turn, action: null, movementUsed, movedPreAction: false, acted, loadoutSwapped: false, reachable: new Map() } });
...
export function resolveRoundBoundary(get: () => GameState, set: any): void {
  ...
  const b = get().battle!;
  set({ battle: { ...b, action: null, movementUsed: 0, movedPreAction: false, acted: false, loadoutSwapped: false, reachable: new Map() }, pendingRoundStart: { round: b.round } });
}

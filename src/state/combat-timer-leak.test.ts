...
import { scheduleCombatTimer, clearCombatTimers } from './combatTimers';
 ...
 * pendant le test SUIVANT du même worker et mute son état. Le filet est `clearCombatTimers()` en
 * `afterEach` de `src/test-setup.ts` — ces deux tests reproduisent le scénario EXACT (armer sans
 * attendre puis vérifier, dans le test SUIVANT, que rien ne s'est déclenché).
 */
describe('scheduleCombatTimer / clearCombatTimers — filet anti-fuite (#405)', () => {
  it('clearCombatTimers annule un timer réel encore en vol avant son échéance', async () => {
    let fired = false;

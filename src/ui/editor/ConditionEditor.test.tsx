...
describe('condSummary — résumé humain de l’algèbre de Condition', () => {
  it('rend chaque forme (flag / horaire / ET / OU / NON) en clair', () => {
    ...
    expect(condSummary({ kind: 'hasItem', trapping: 'Clé en fer' })).toContain('Clé en fer');
...
describe('ConditionEditor — conditions d’état VIVANT (objet / bourse / mort)', () => {
  it('hasItem rend le champ objet + le compte', () => {
    const html = renderToStaticMarkup(<ConditionEditor cond={{ kind: 'hasItem', trapping: 'Amulette', count: 2 }} onChange={() => {}} />);
    expect(html).toContain('value="Amulette"');

...
describe('SeatAssignmentsField — authoring des places assises (id-only)', () => {
  it('affiche les slots de la ref et écrit un occupant par id', () => {
    ...
    expect(ui.sceneOf().seatAssignments).toEqual({ 'table-1': { 'place-nord': { kind: 'entity', entityId: 'pnj-aubergiste' } } });
  });
  ...
  it('rasseoir un PNJ déjà attablé le DÉPLACE de place — jamais deux places pour un corps', () => {
    ...
    expect(ui.sceneOf().seatAssignments).toEqual({ 'table-1': { 'place-est': { kind: 'entity', entityId: 'pnj-aubergiste' } } });
  ...
  it('assigne un EMPLACEMENT par rang, et le déplacer d’une place à l’autre est ACCEPTÉ', () => {
    ...
    expect(ui.sceneOf().seatAssignments).toEqual({ 'table-1': { 'place-nord': { kind: 'party', rang: 2 } } });
    ui.choose('place-sud', 'party:2');
    expect(ui.sceneOf().seatAssignments).toEqual({ 'table-1': { 'place-sud': { kind: 'party', rang: 2 } } });
  ...
  it('libérer un emplacement le laisse proposable — la liste ne dépend pas de l’occupation', () => {
    ...
    ui.choose('place-nord', 'party:1');
    expect(ui.sceneOf().seatAssignments).toEqual({ 'table-1': { 'place-nord': { kind: 'party', rang: 1 } } });
  });

...
/** Position VISUELLE d'un sujet à l'instant du rendu. */
...
/** Pose à l'instant `now`. Sans marche vivante, le sujet est à sa case et ne glisse pas ; sinon sa
 *  position est FRACTIONNAIRE le long du chemin. */
export function walkPoseAt(track: WalkTrack | undefined, x: number, y: number, now: number): WalkPose {
  if (!track) return { x, y, walking: false };
  const p = walkXY(track.path, now - track.start, STEP_MS);
...
export function walkGlideM(
  ...
  const p = walkPoseAt(track, base.x, base.y, now);

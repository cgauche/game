...
/** BUILDER des étiquettes de zone descriptive : gate d'ÉTAGE (#804), même patron que `buildProps`
 *  (`props.test.ts` l.60) — `activeZ`/`viewZ` ABSENTS ⇒ toutes les couches ; fournis ⇒ `viewZ` isole
 *  un étage, sinon seules les couches ≤ `activeZ` s'affichent. */

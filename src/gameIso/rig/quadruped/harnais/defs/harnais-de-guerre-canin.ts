...
// HARNAIS DE GUERRE CANIN — le harnachement complet du molosse de l'artwork officiel
// (art-ref/ldb/page317_img7361.png, statblock Chien LDB 315) : caparaçon de cuir garrot→hanche à
// liseré d'acier riveté, chaînes de croupe, plaque de croupe segmentée en lames, sangles bouclées
// de passage et de ventre, plastron d'épaule riveté et plaque de bras, gorgerin d'acier à la base
// du cou, collier à pointes et plaque à tête de mort sur la gorge.
//
// L'art vit dans les COORDONNÉES LOCALES des os `tronc` et `encolure` du gabarit `chien`
// (`girth` 1,52 / `bodyLen` 0,92 / `neckLen` 0,36 cuits dans ses tracés) : d'où
// `especes: ['chien']` — posé sur une autre carrure, il glisserait. La clé de tronc est NUE (l'os
// porte son art dans les trois vues) ; le gorgerin vise `encolure#profile` SEULEMENT — l'os
// `encolure` ne porte d'art qu'en profil, une clé nue y réclamerait deux vues qu'elle ne peint pas.
// Elle se ré-ouvrira le jour où un art de collier vu de bout existera. `plan: 0` = le plan de l'os
// lui-même : le fragment se peint APRÈS l'art de la bête, exactement comme la déco d'espèce le
// faisait. Cuir du harnachement en jeton PROPRE (`@harnaisCuir`) : `@cuir` habille aussi les
// coussinets du chien, recolorier l'un ne doit pas déteindre sur l'autre.
export const quadHarnais: QuadHarnaisDef = {
  id: 'harnais-de-guerre-canin',
  label: 'Harnais de guerre canin',
  especes: ['chien'],
  deco: {
    // repère local du tronc (profil) : +x = avant, dos y≈-19, ventre y≈+10 (bodyLen 0.92)
    tronc: [{ plan: 0, svg: `<g data-deco="harnais">` +
      // caparaçon de cuir garrot→hanche
      `<path d="M14 -18.5 Q-4 -21 -24 -16.5 Q-27 -8 -25 -1.5 Q-6 -7 13 -5.5 Q15.5 -12 14 -18.5 Z" fill="@harnaisCuir" stroke="#0e0b07" stroke-width="0.6"/>` +
      ...
      `<path d="M11 -5 L14 -5 Q15 4 12.5 11 L9.5 11 Q12 4 11 -5 Z" fill="@harnaisCuir" stroke="#0e0b07" stroke-width="0.5"/>` +
      `<rect x="10.6" y="1.5" width="3.4" height="3.4" rx="0.6" fill="@accent" stroke="@accentO" stroke-width="0.4"/>` +
      // sangle de ventre arrière + clou
      `<path d="M-24.5 0 L-21.5 -0.5 Q-20 5.5 -21.5 10 L-24.5 10 Q-23 5 -24.5 0 Z" fill="@harnaisCuir" stroke="#0e0b07" stroke-width="0.5"/>` +
      `<circle cx="-22.8" cy="4.8" r="0.9" fill="@accent" stroke="@accentO" stroke-width="0.35"/>` +
      // PLASTRON d'épaule : dôme riveté sur l'épaule/le poitrail VISIBLE (à y -17..2, il vivait
      // sous la tête z7 → invisible au rendu) + plaque de bras
      ...
      `</g>` }],
    // GORGERIN d'acier à la BASE du cou (à y -9..-1, le collier vivait sous la tête z7 →
    ...
    'encolure#profile': [{ plan: 0, svg: `<g data-deco="collier">` +
      ...
      `<path d="M-9.4 -0.4 Q0 2 9.8 -1 L9 -5 Q0 -2.6 -8.8 -5.2 Z" fill="@harnaisCuir" stroke="#0e0b07" stroke-width="0.6"/>` +

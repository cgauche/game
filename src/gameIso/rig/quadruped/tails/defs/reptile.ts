...
export const quadTail: QuadTailDef = {
  ...
  art: {
    ...
    profile: (p) => {
      // Bête à crête CONTINUE (`tailCrest` — hydre) / dorsale 'epines-continues' (basilic) : la
      // crête @cheveux se PROLONGE sur la queue jusqu'à la pointe (artwork).
      const crest = p.tailCrest || p.ridge === 'epines-continues'

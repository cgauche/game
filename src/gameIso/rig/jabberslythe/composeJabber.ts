...
// Queue annelée finie en DARD de scorpion (bulbe + crochet corné) — le trait le plus distinctif.
function tailSting(rootX: number, rootY: number, dir: number): string {
  ...
  return `<g>` +
    `<path d="M${x(0)} ${rootY - 4} C${x(16)} ${rootY + 2} ${x(28)} ${rootY - 4} ${x(30)} ${rootY - 14} C${x(31)} ${rootY - 22} ${x(26)} ${rootY - 28} ${x(20)} ${rootY - 30} L${x(18)} ${rootY - 25} C${x(23)} ${rootY - 23} ${x(26)} ${rootY - 19} ${x(25)} ${rootY - 14} C${x(23)} ${rootY - 7} ${x(14)} ${rootY + 4} ${x(0)} ${rootY + 4} Z" fill="@corps" stroke="@corpsO" stroke-width="0.7"/>` +
    // anneaux clairs (bandes de l'artwork)
    `<path d="M${x(8)} ${rootY - 1} l${(d * 1.5).toFixed(1)} 5 M${x(15)} ${rootY - 2.5} l${(d * 2).toFixed(1)} 5 M${x(22)} ${rootY - 6} l${(d * 3).toFixed(1)} 3.5 M${x(27)} ${rootY - 12} l${(d * 4).toFixed(1)} 1.5" stroke="@corpsH" stroke-width="0.9" opacity="0.55"/>` +
    // bulbe + crochet du dard
    `<ellipse cx="${x(19)}" cy="${rootY - 29}" rx="3.4" ry="2.9" fill="@corps" stroke="@corpsO" stroke-width="0.7"/>` +
    `<path d="M${x(17)} ${rootY - 31.5} Q${x(11)} ${rootY - 37.5} ${x(6)} ${rootY - 36} Q${x(7.5)} ${rootY - 33.5} ${x(10.5)} ${rootY - 32.5} Q${x(13)} ${rootY - 31.5} ${x(14)} ${rootY - 27.5} Z" fill="@cuir" stroke="#1a140e" stroke-width="0.5"/>` +
    `<path d="M${x(9)} ${rootY - 36.2} Q${x(6.5)} ${rootY - 36.4} ${x(6)} ${rootY - 36} L${x(7.5)} ${rootY - 33.8} Z" fill="#1a140e"/>` + // pointe sombre du dard
    `</g>`;
}
...
function head(p: JabberProps, view: View): string {
  ...
  if (view === 'profile') {
    ...
    return `<g>${horns}${antlers}` +
      ...
      `<path d="M6 2.6 l0.9 2.8 l1.3 -2.5 M9 2.9 l0.9 2.7 l1.3 -2.3 M12 3 l0.8 2.4 l1.2 -2.1 M5 10.6 l0.8 -2.6 l1.2 2.3 M8.5 11.8 l0.8 -2.6 l1.2 2.3" stroke="#efe6cf" stroke-width="0.8" fill="none"/>` + // crocs haut + bas
      tongueP +
      `<ellipse cx="1.5" cy="-3.5" rx="2.2" ry="2.4" fill="#f2e84a"/><ellipse cx="1.8" cy="-3.3" rx="0.6" ry="1.9" fill="#0a0603"/>` +
      `<path d="M-2 -6 Q2 -7.5 5 -5.6" stroke="@corpsO" stroke-width="1.1" fill="none"/></g>`;
  }

...
describe('registre de créatures (auto-collecté depuis defs/)', () => {
  it('CREATURES non vide + chaque entrée bien formée', () => {
    ...
    const PLANS = ['biped', 'quadruped', 'winged', 'serpentine', 'arachnid', 'avian', 'cephalopod', 'spectral', 'squig', 'monolithic'];
    const PROPS: Record<string, keyof typeof CREATURES[number]> = {
      quadruped: 'quad', winged: 'quad', serpentine: 'serpent', arachnid: 'spider', avian: 'bird', cephalopod: 'octopus', spectral: 'spectre', squig: 'squig',
    };

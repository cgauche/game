 ...
 * anti-perte (même marche que les slots), la complétude du schéma de projet, et l'unicité prouvée à
 * la porte.
 */
...
const LISTES_DE_REFERENCES: Readonly<Record<string, string>> = {
  '.scenes[].effectZones[].crossTest.gate.of[]': 'conditions composées : `id` d’un objet/talent désigné',
  ...
  '.worldMap.places[].when.of[]': 'conditions composées : `id` d’un objet/talent désigné',
...
function clesDeLElement(noeud: unknown, vus = new Set<unknown>()): string[] {
  ...
  const enveloppe = enfantsDe(def).find((e) => e.segment === '');
...
function listesAId(schema: unknown): { chemin: string; cle?: string }[] {
  ...
  const marcher = (noeud: unknown, chemin: string): void => {
    if (!noeud || vus.has(noeud)) return;
    vus.add(noeud);
    const def = defDe(noeud);
    if (!def) return;
    if (def.type === 'array' && clesDeLElement(def.element).includes('id')) out.push({ chemin: `${chemin.replace(/\|\d+/g, '')}[]`, cle: cleDe(noeud)?.nom });
    for (const e of enfantsDe(def)) marcher(e.noeud, chemin + e.segment);
  };
  marcher(schema, '');
  return out;
}

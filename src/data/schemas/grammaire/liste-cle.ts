 ...
 * son rang.
 *
 * Registre DISTINCT de `slots.ts` : un slot désigne une RÉFÉRENCE vers un autre document (`slotsDe`
 * les compte comme clés étrangères), une clé d'élément désigne l'IDENTITÉ d'un élément dans SA liste.
 * La marche est la même (`visiterNoeuds`), et le même piège vaut : la marque vit sur le nœud RENDU
 * par la fabrique ; un `.min`/`.refine` posé APRÈS le clone et la perd — le compteur anti-perte
 * (`clesPosees`/`clesRetrouvees`) le nomme.
 */
import { z } from 'zod';
import './locale-fr';
import { visiterNoeuds } from './slots';
...
/** Listes à clé RETROUVÉES par la marche d'un schéma — la face « recensement », même marche que les slots. */
export function clesRetrouvees(schema: unknown): Set<object> {
  const trouvees = new Set<object>();
  visiterNoeuds(schema, (noeud) => {

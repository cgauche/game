 ...
 * utilitaires/narratifs (lumière, bruits, serrure…) : leur EFFET `narrative`
 * VERBATIM-résumé vit désormais sur `SpellData.effects` (Flow, donnée app-owned).
 * La spec ne garde que les métadonnées de résolution. Le `text` passé à `N` reste
 * la trace DOCUMENTAIRE de l'effet (recopié dans le seed `effects`).
 */
import { SpellSpec } from '../../engine/spellspec';

// `text` = trace documentaire de l'effet narratif (désormais sur `effects`) — conservé pour la relecture.
const N = (label: string, _text: string, durationRounds: SpellSpec['durationRounds'] = null): SpellSpec => ({
  label,
  durationRounds,
  curated: true,
...
export const MAGIE_MINEURE: SpellSpec[] = [
  ...
  {
    label: 'Fléchette',
// Projectile magique +0 — entièrement résolu par le moteur missile.
    durationRounds: null,
    curated: true,

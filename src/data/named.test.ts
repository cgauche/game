...
/** Garde de DONNÉE : les 30 individus nommés validés contre le RAW (desc verbatim singulier = individu)
...
describe('creatures.json — backfill `named`', () => {
  ...
  it('les 30 nommés validés portent named:true (ids résolus)', () => {
    const bad = NAMED_IDS.filter((id) => { const c = findCreatureById(id); return !c || !isNamed(c); });
    expect(bad).toEqual([]);
  });
  it('exactement 30 nommés dans la base (zéro générique flaggé)', () => {
    expect(creatures.filter(isNamed).length).toBe(30);
  });

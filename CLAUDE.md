...
| **Menu** (menu PRINCIPAL hors partie ET tiroir du menu ☰ EN jeu) : carte + sections de grands boutons pleine largeur icône+libellé + séparateurs titrés + interrupteur de menu — *réflexe avant tout `.menu-card` ou `<button className="btn">` de menu recodé* | `MenuCard` (+ `MenuSection`/`MenuButton`/`MenuToggle`) | `src/ui/MenuCard.tsx` |
| **Méta d'en-tête** date+bourse `{time?, money?}` (`.hud-clock` + `.port-purse`) — partagée en-tête d'écran plein-champ ET en-tête du menu ☰ | `ScreenMeta` (composée par `ScreenShell` et `GameMenu`) | `src/ui/ScreenMeta.tsx` |

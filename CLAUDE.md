...
| **Présence PLEIN FORMAT** d'un héros hors combat (colonne aside de la fiche) — boîte-figurine `FigTile` en variante STATIQUE (`fig="hero"`, sans `onClick`/label : aucune sémantique de picker), rig 320px sur sa lueur de sol | `FigTile fig="hero"` (même primitive que la table ci-dessus, sans nouveau composant) | `src/ui/FigTile.tsx` |
| **Arc de vie** (jauge de Blessures gravée, SOUS une boîte-figurine) : arc SVG teinté en CONTINU du plein (or) au critique (sang) par le ratio courant/max (`color-mix`, tokens `--gold2`/`--danger` existants — jamais de seuil arbitraire), légende "N / M Blessures" intégrée au tracé | `VitalArc` | `src/ui/VitalArc.tsx` |

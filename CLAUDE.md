...
|---|---|---|
| Coquille d'**écran plein-champ** (carte du monde, port/escale, marché, dossier de navire, négoce) — voile + en-tête (titre/actions/fermeture) + corps, a11y de dialogue, barre d'outils `.screen-toolbar` optionnelle (y poser `<Tabs>`) | `ScreenShell` (jamais recopier `.worldmap-overlay`/`.worldmap-head`) | `src/ui/ScreenShell.tsx` |

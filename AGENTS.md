...
npm run ops:chantier -- <N>   # ouvre le chantier <N> : worktree lié `.wt-<N>` sur origin/main, branche `chantier/<N>`, `npm ci`, port dev dérivé — imprime worktree/branche/base/port
npm run ops:publier -- --detache   # joue le TRAIN ENTIER depuis le worktree courant : rebase sur origin/main → docs dérivés régénérés et commités → gates `--serie` détachées → push HEAD:main → sonde de la course CI → pilotage des tickets cités ; journal `node_modules/.cache/publication/<branche>.json` + `.log` à côté, `pid=`/`log=` imprimés ; `--reprendre` repart de la première étape non verte, `--etapes` n'imprime que le plan ; veille = Monitor sur `^PUBLICATION:` dans le log
npm run ops:worktrees [-- --purger]   # inventaire des worktrees liés (branche, propreté, fusion dans origin/main) ; `--purger` ne retire que les PROPRES et FUSIONNÉS
...
src/state/    store Zustand + flux (combatFlow barils, rollFlowFactory/Specs, scene.ts = SCHÉMA, upkeep…) ; asciiMap.ts = authoring ASCII
...
src/scenes/   documents de scène + campagne Arène (arene-projet.json)
